# Arcus AI — Full-Stack Audit (2026-05-28)
...
## SECTION 1 — Chat agentic loop (`lib/arcus/loop.ts`)
...
## SECTION 2 — Tool layer (`lib/arcus/tools.ts`, 110 tools, 9241 lines)
...
## SECTION 3 — Background-agent runtime (`lib/arcus/run-agent.ts` +
...
### F3.5 🟨 `fetchUserInstructions` doesn't respect `arcus_instructions_enabled`
...
But the field is misnamed — `arcus_personality` holds rules now, not
personality. Two separate fields would be cleaner.

**Fix:** schema migration → split `arcus_personality` (voice writing
style) from `arcus_user_instructions` (binding rules). Until migrated,
keep the field as-is.

---

## SECTION 4 — System prompts (`lib/arcus/system-prompt.ts`)
...
**Fix:** new `arcus_agent_runs` table — one row per cron tick that
