  ...
  async _runLoop(userMessage, controller) {
    ...
    while (this.iteration < MAX_ITERATIONS) {
      ...
      emit('thinking', {
        ...
        status: 'active',
        step: this.iteration === 1 ? 'Analysing your request...' : `Step ${this.iteration}: Processing findings...`
      });
      ...
      if (toolCall) {
        ...
        if (name === 'respond') {
          const cleanMessage = params.message || params.content || (typeof rawContent === 'string' ? rawContent.replace(/<tool_call>[\s\S]*?<\/tool_call>/i, '').trim() : '');
          emit('message', {
