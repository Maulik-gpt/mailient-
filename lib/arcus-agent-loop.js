...
Body: ${email.body?.substring(0, 5000)}
    ...
    while (this.iteration < MAX_ITERATIONS) {
      ...
      // KEEP-ALIVE: Send high-frequency pings every 3s to prevent Vercel 10s timeout
      const aiCallPing = setInterval(() => {
        try {
          controller.enqueue(new TextEncoder().encode(sseEvent('ping', { iteration: this.iteration, status: 'reasoning' })));
        } catch (e) {
          clearInterval(aiCallPing);
        }
      }, 3000);
      ...
      if (toolCall) {
        ...
        if (name === 'respond') {
          ...
          let isDraft = /subject:|to:|dear|hello|best regards|sincerely|---\n/i.test(cleanMessage) && 
          ...
          if (saveDraftStep && saveDraftStep.params) {
            ...
            draftData = {
              ...
              subject: saveDraftStep.params.subject || 'Draft Reply',
              recipientEmail: saveDraftStep.params.to || '',
            ...
            draftData = {
              ...
              subject: subjectMatch ? subjectMatch[1] : 'Draft Reply',
              recipientEmail: toMatch ? toMatch[1] : ''
            };
          }
