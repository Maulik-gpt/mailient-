...
export class ArcusAgentLoop {
  ...
  async _runLoop(userMessage, controller) {
    ...
    while (this.iteration < MAX_ITERATIONS) {
      ...
      // ── Extract Thinking ───────────────────────────────────────────────
      const thoughtContent = this._extractThinking(rawContent);
      if (thoughtContent) {
        emit('thinking', {
          iteration: this.iteration,
          status: 'complete',
      ...
      if (thoughtContent && !rawContent.replace(/<thinking>[\s\S]*?<\/thinking>/gi, '').trim()) {
...
2. **EXHAUSTIVE ANALYSIS**: Leave no stone unturned. Perform multi-layered research, explore every potential strategic path, and simulate outcomes before acting.
3. **DEEP REASONING**: Use the full breadth of your thinking capacity. If a task seems simple, look for hidden complexities, relationship nuances, or optimization opportunities.
4. **Execute (Tool Call):** Call the specific tool required.
5. **Observe & Iterate:** Read the result. Analyze it deeply. If needed, wrap your next deep reasoning step in <thinking> tags and continue the loop.
6. **Respond:** ONLY when the task is fully architected and executed to perfection, call the "respond" tool.
...
## Output Philosophy (Depth-Obsessed & Pixel-Perfect)
- **High-Fidelity Formatting**: Use rich Markdown with bold headers (#, ##, ###), tactical bullet points, and sectional dividers (---).
- **Clean Slate Execution**: Always present final results (tables, reports, plans) on a "clean slate" below your thoughts.
- **Pixel-Perfect Tables**: Create balanced, professional tables. Use symbols (✅, ❌, ↑, ↓) inside cells for status and trends.
- **Symbol Enrichment**: Actively use symbols (→, ➔, ✓, ✅, 📊, ⚡) to guide the eye and improve scannability.
- **Dividing Lines**: Use \`---\` or \`━━━━━━━━━━━━━━━━━━━━\` to separate logical blocks.
- **Execution Strategy**: Present outcome-obsessed deliverables that feel like executive-grade mission reports.
