...
export class VoiceProfiler {
  ...
  static generatePromptFragment(signals, fuzzyVibe = {}) {
    ...
    const { tone = "professional", slang = "none", formality_score = 5, avg_reply_sentences = 4, punctuation_style = "standard", vocabulary_level = "moderate", structural_habit = "short-paragraphs" } = fuzzyVibe;
...
- Punctuation: ${punctuation_style}${vibe.exclamatory ? ', uses exclamation marks' : ''}
- Structure: ${structural_habit}, ~${avg_reply_sentences} sentences per reply
- Emojis: ${emojis.frequency > 0 ? `${emojis.top.join(' ')}` : 'never uses'}
- Common fillers/slang: ${[...fillers, ...(slang !== 'none' ? [slang] : [])].join(', ') || 'none'}
