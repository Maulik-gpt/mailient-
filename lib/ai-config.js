...
export { DEFAULT_AI_MODELS, getModelChain };
...
export class AIConfig {
  ...
  getConfigurationStatus() {
    ...
    status.push({
      ...
      model: process.env.OPENROUTER_MODEL || DEFAULT_AI_MODELS[0],
   ...
   */
  async generateChatResponse(userMessage, emailContext = null, sessionOrPrivacyMode = null, privacyMode = false) {
    if (!this.openrouterService) {
      throw new Error('OpenRouter service not available');
    }

    let actualPrivacyMode = privacyMode;
    if (typeof sessionOrPrivacyMode === 'boolean') {
      actualPrivacyMode = sessionOrPrivacyMode;
    }

    return await this.openrouterService.generateChatResponse(userMessage, emailContext, actualPrivacyMode);
  }
  ...
  async generateEmailSummary(emailContent, privacyMode = false, userContext = {}, options = {}) {
    ...
    return await this.openrouterService.generateEmailSummary(emailContent, privacyMode, userContext, options);
  }
  ...
  async generateDraftReply(emailContent, category, userContext = {}, privacyMode = false, options = {}) {
    ...
    return await this.openrouterService.generateDraftReply(emailContent, category, userContext, privacyMode, options);
  }
  ...
  /**
   * Specialized streaming for drafts
   */
  async generateDraftStream(emailContent, category, userContext = {}, privacyMode = false) {
    if (!this.openrouterService) {
      throw new Error('OpenRouter service not available');
    }

    // This returns a ReadableStream
    return await this.openrouterService.callOpenRouterStream([
      { role: 'system', content: `You are an elite email assistant. Draft a ${category} reply.` },
      { role: 'user', content: emailContent }
    ], { temperature: 0.7, privacyMode });
  }
