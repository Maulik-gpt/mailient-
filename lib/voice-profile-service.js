...
Respond ONLY with a compact JSON object:
{
  ...
  "slang": "comma-separated filler words or slang they actually use, or 'none'",
  "personality": "direct/warm/analytical/enthusiastic/reserved",
  "formality_score": 1-10,
  "avg_reply_sentences": 2-8,
  "punctuation_style": "minimal/standard/expressive",
  "vocabulary_level": "simple/moderate/sophisticated",
  "structural_habit": "one-liner/short-paragraphs/bullet-points/detailed-prose"
}

Do NOT describe email content. Describe ONLY how they write.`
                    },
                    {
                        role: 'user',
                ...
                const response = await this.callOpenRouter(messages, { maxTokens: 250, temperature: 0.1 });
                if (response?.choices?.[0]?.message?.content) {
                    const content = response.choices[0].message.content.trim();
                    const parsed = JSON.parse(content.replace(/```json|```/g, '').replace(/<think>[\s\S]*?<\/think>/gi, '').trim());
                    fuzzyVibe = {
                        tone: parsed.tone || 'professional',
                        slang: parsed.slang || 'none',
                        personality: parsed.personality || 'balanced',
                        formality_score: parsed.formality_score || 5,
                        avg_reply_sentences: parsed.avg_reply_sentences || 4,
                        punctuation_style: parsed.punctuation_style || 'standard',
                        vocabulary_level: parsed.vocabulary_level || 'moderate',
                        structural_habit: parsed.structural_habit || 'short-paragraphs'
                    };
                }
            } catch (llmError) {
...
`;
    ...
    buildManualPromptFragment(manualSettings, existingProfile = {}) {
        ...
        // Map slider values (0-100) to descriptive labels
        const formalityLabel = tone.formality > 70 ? 'formal' : tone.formality > 40 ? 'semi-formal' : 'casual';
        const detailLabel = tone.detail > 70 ? 'detailed and thorough' : tone.detail > 40 ? 'balanced' : 'brief and concise';
        const warmthLabel = tone.warmth > 70 ? 'direct and matter-of-fact' : tone.warmth > 40 ? 'balanced warmth' : 'warm and friendly';
        const confidenceLabel = tone.confidence > 70 ? 'confident and assertive' : tone.confidence > 40 ? 'balanced' : 'reserved and measured';

        let fragment = `User Voice Profile (Mimic this style precisely):
- Formality: ${formalityLabel} (${tone.formality || 50}/100)
- Detail level: ${detailLabel} (${tone.detail || 40}/100)
- Warmth/Directness: ${warmthLabel} (${tone.warmth || 50}/100)
- Confidence: ${confidenceLabel} (${tone.confidence || 30}/100)`;
    ...
    async generatePreviewReply(manualSettings, sampleEmail) {
        ...
        const messages = [
            {
                role: 'system',
                content: `You are generating a SHORT sample email reply (1-2 sentences max) to demonstrate a user's writing style. 
${promptFragment}

Write a VERY SHORT reply (max 2 sentences) to the sample email below. Match the voice profile EXACTLY. 
Do NOT add subject lines, greetings, or sign-offs unless the habits specifically mention them. 
Output ONLY the reply text, nothing else.`
            },
            {
                role: 'user',
                content: `Sample email to reply to: "${sampleEmail}"\n\nWrite the short reply now.`
            }
        ];

        try {
            const response = await this.callOpenRouter(messages, { maxTokens: 100, temperature: 0.4 });
            const content = response?.choices?.[0]?.message?.content?.trim();
            if (content) {
