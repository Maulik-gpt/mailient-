...
export class SubscriptionService {
    ...
    async getUserPlanType(userId) {
        ...
        const mapping = { 'starter': 'starter', 'pro': 'pro', 'professional': 'pro', 'basic': 'starter', 'premium': 'pro' };
    ...
    async getFeatureUsage(userId, featureType) {
        if (!userId) return { usage: 0, limit: 0, remaining: 0, hasAccess: false, planType: 'none' };
        userId = userId.toLowerCase();
        try {
            const subscription = await this.getUserSubscription(userId);
            ...
            return { usage: 0, limit: 0, remaining: 0, hasAccess: false, planType: 'none' };
        }
    }
    ...
    async incrementFeatureUsage(userId, featureType) {
        ...
        try {
            ...
            const periodStart = subscription ? subscription.subscription_started_at.split('T')[0] : todayStr;
            const periodEnd = subscription ? subscription.subscription_ends_at.split('T')[0] : todayStr;
    ...
    async getAllFeatureUsage(userId) {
        if (!userId) return { hasActiveSubscription: false, planType: 'none', features: {} };
        userId = userId.toLowerCase();
        try {
            const subscription = await this.getUserSubscription(userId);
            const planType = await this.getUserPlanType(userId);
