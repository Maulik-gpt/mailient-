...
export class SubscriptionService {
    ...
    async getUserPlanType(userId) {
        ...
        const subscription = await this.getUserSubscription(userId);
        if (!subscription) {
            console.log(`📋 [PLAN] No subscription found for ${userId}, defaulting to free`);
            return 'free';
        }
        ...
        if (!validStatuses.includes(subscription.status)) {
            console.log(`📋 [PLAN] Invalid status '${subscription.status}' for ${userId}, defaulting to free`);
            return 'free';
        }
        ...
        if (subscription.subscription_ends_at) {
            ...
            const endDate = new Date(subscription.subscription_ends_at);
            if (endDate <= now) {
                console.log(`📋 [PLAN] Subscription expired for ${userId} (ended ${subscription.subscription_ends_at}), defaulting to free`);
                return 'free';
            }
        }
