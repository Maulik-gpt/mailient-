...
export class SubscriptionService {
    ...
    async getFeatureUsage(userId, featureType) {
        try {
            ...
            // For Starter users, we check their active status
            ...
            return { usage: 0, limit: 0, remaining: 0, hasAccess: false, reason: error.message, planType: 'starter' };
        }
    }
    ...
    async getAllFeatureUsage(userId) {
        try {
            const subscription = await this.getUserSubscription(userId);
            const planType = subscription?.status === 'active' ? subscription.plan_type : 'none';
