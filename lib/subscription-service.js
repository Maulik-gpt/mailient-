...
export const PLANS = {
    free: {
        ...
        limits: {
            ...
            openai_tokens: { limit: 50000, period: 'monthly' } // 50k tokens free tier
        }
    },
    starter: {
        id: 'starter',
        ...
        limits: {
            ...
            openai_tokens: { limit: 500000, period: 'monthly' } // 500k tokens starter
        }
    },
    pro: {
        id: 'pro',
...
export class SubscriptionService {
    ...
    async getUserPlanType(userId) {
        ...
        // Expiration check - only if end date exists
    ...
    async getFeatureUsage(userId, featureType) {
        ...
        try {
            ...
            if (isDaily) {
                ...
                // Define usage window based on subscription period
                // Use a far future date if ends_at is missing to avoid querying 1970
                const subStart = new Date(subscription.subscription_started_at || '2020-01-01').toISOString().split('T')[0];
                const subEnd = subscription.subscription_ends_at 
                    ? new Date(subscription.subscription_ends_at).toISOString().split('T')[0]
                    : '2099-12-31';
            ...
            return {
                id: row?.id, // Return ID for targeted updates
                usage,
    ...
    async incrementFeatureUsage(userId, featureType) {
        ...
        try {
            ...
            // If we found an existing row by ID, update it specifically
            // Otherwise use upsert with onConflict for new rows
            let updateResult;
            if (currentUsage.id) {
            ...
            return { success: !updateResult.error, newUsage: currentUsage.usage + 1 };
        } catch (error) {
    ...
    async getRecentPayments(userId) {
        const plan = await this.getUserPlanType(userId);
        if (plan === 'none') return [];
        
        // Mocking real payment data since we don't have a payments table yet
        const now = new Date();
        const payments = [];
        
        // Add current month payment/activation
        payments.push({
            id: `pay_${Math.random().toString(36).substr(2, 9)}`,
            amount: plan === 'starter' ? 7.99 : plan === 'pro' ? 29.99 : 0.00,
            currency: 'USD',
            status: 'succeeded',
            date: new Date(now.getFullYear(), now.getMonth(), 1).toISOString(),
            method: plan === 'free' ? 'Activation' : 'Visa **** 4242'
        });

        // Add previous month if applicable for paid plans
        if (plan !== 'free') {
            payments.push({
                id: `pay_${Math.random().toString(36).substr(2, 9)}`,
                amount: plan === 'starter' ? 7.99 : 29.99,
                currency: 'USD',
                status: 'succeeded',
    ...
    async getInvoices(userId) {
        const payments = await this.getRecentPayments(userId);
        return payments.map(p => ({
            id: `inv_${p.id.split('_')[1]}`,
            paymentId: p.id,
            number: `ML-${new Date(p.date).getFullYear()}-${Math.floor(Math.random() * 9000) + 1000}`,
            date: p.date,
            amount: p.amount,
            status: 'paid',
            downloadUrl: '#' // In a real Polar integration, this would be a signed URL
        }));
    }
