...
export class SubscriptionService {
    ...
    async incrementFeatureUsage(userId, featureType, runId = null) {
        ...
        try {
            ...
            // or if the same runId is provided twice
            const now = new Date();
            if (currentUsage.updatedAt) {
                ...
                // If the same runId is used (and it matches what we might have tracked)
                // or if it's within a very tight 3-second window
                if (diffSeconds < 3 && !runId) {
                    console.log(`🛡️ [SERVICE] Throttling rapid increment for ${userId} : ${featureType}`);
            ...
            const usageObj = {
                ...
                updated_at: now.toISOString(),
                // Store the last runId to specifically deduplicate those
                metadata: {
                    lastRunId: runId,
