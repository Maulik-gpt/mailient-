...
export const PLANS = {
    starter: {
        ...
        name: 'Starter',
        price: 7.99,
        // Polar product/checkout identifiers
...
export class SubscriptionService {
    ...
    async testConnection() {
        ...
        try {
            ...
            // Try to query the user_subscriptions table
            ...
            if (error) {
                ...
                if (error.code === '42P01') {
                    console.error('❌ CRITICAL: user_subscriptions table does not exist in Supabase!');
                    console.error('❌ Please run the supabase-subscriptions-migration.sql script in your Supabase SQL editor.');
                    return false;
                }
    ...
    async isSubscriptionActive(userId) {
        ...
        // Allow active status OR cancelled status (if period hasn't ended)
        ...
        // Even if cancelled, it's active if the end date is in the future
        return endDate > now;
    }
    ...
    async getUserPlanType(userId) {
        ...
        // Allow active status OR cancelled status (if period hasn't ended)
        if (subscription.status !== 'active' && subscription.status !== 'cancelled') return 'none';
        ...
        // Enhanced plan type normalization and validation
        if (planType) {
            // Remove whitespace and convert to lowercase
            planType = planType.toString().trim().toLowerCase();

            // Map common variations to standard plan types
            ...
            const normalizedPlanType = planTypeMapping[planType] || planType;

            // Validate that the plan type exists in PLANS
            if (PLANS[normalizedPlanType]) {
                console.log(`✅ Plan type normalized: ${subscription.plan_type} -> ${normalizedPlanType}`);
                return normalizedPlanType;
            } else {
                console.error('❌ Invalid plan type after normalization:', {
                    original: subscription.plan_type,
        ...
        return subscription.plan_type || 'none';
    }
     ...
     * @param {string} subscriptionId - External subscription/order ID (from Polar/Whop)
     * @param {Object} providerDates - Optional dates from payment provider (validUntil, createdAt)
     */
    async activateSubscription(userId, planType, subscriptionId = null, providerDates = null) {
        ...
        try {
            ...
            // If this came from a renewal webhook, ALWAYS treat it as a renewal
            // so that usage is preserved for the next month
            ...
            if (existingSubscription) {
                ...
                if (isWebhookRenewal) {
                    // Renewal from provider → preserve usage here
                    isNewSubscription = false;
                    console.log(`🔁 Renewal detected for ${userId}, preserving usage in activateSubscription`);
                } else if (isActive && existingEndDate > now) {
                    // Mid-period update (e.g., plan change) → preserve usage
                    isNewSubscription = false;
                    console.log(`🔒 User ${userId} has existing active subscription, preserving usage`);
                } else {
                    // Previous subscription is expired / inactive → this is a fresh billing cycle
                    // Treat as NEW subscription so the user gets a clean set of credits.
                    isNewSubscription = true;
                    console.log(`🆕 New billing cycle for ${userId} (previous subscription not actively within period)`);
                }
            }

            // CRITICAL FIX: Use dates from provider webhook if available
            ...
            if (providerDates) {
                console.log('🔄 Using provider-provided dates:', providerDates);

                // Use provider's creation date as start date if available
                if (providerDates.createdAt) {
                    startDate = new Date(providerDates.createdAt * 1000); // Unix timestamps
                }

                // MOST IMPORTANT: Use provider's valid_until as the end date
                if (providerDates.validUntil) {
                    endDate = new Date(providerDates.validUntil * 1000); // Unix timestamps
                    console.log('✅ Renewal date from provider:', endDate.toISOString());
                } else if (providerDates.expiresAt) {
                    endDate = new Date(providerDates.expiresAt * 1000);
                    console.log('✅ Expiry date from provider:', endDate.toISOString());
                }
            } else {
                console.warn('⚠️ No provider dates provided, calculating locally (should only happen for manual activation)');
            }
            ...
            console.log('💾 Saving subscription:', {
                ...
                isNewSubscription,
                source: providerDates ? 'Provider' : 'Calculated'
            });
            ...
            console.log(`✅ Subscription ${isNewSubscription ? 'activated' : 'updated'} for ${userId}: ${planType}, renews at ${endDate.toISOString()}`);
            return data;
        } catch (error) {
    ...
    async resetAllFeatureUsage(userId, periodStart, periodEnd, isNewSubscription = true) {
        ...
        if (!isNewSubscription) {
            console.log(`🔒 Preserving existing usage for ${userId} (renewal/re-auth)`);
            return;
        }
        ...
        try {
            ...
            console.log(`🔄 Reset feature usage for NEW subscription ${userId}`);
        } catch (error) {
    ...
    async handleMonthlyResetForRenewal(userId, newPeriodStart, newPeriodEnd) {
        ...
        try {
            ...
            if (monthlyFeatures.length > 0) {
                ...
                console.log(`🔄 Reset monthly features for ${userId}: ${monthlyFeatures.map(f => f.feature_type).join(', ')}`);
            }
            ...
            if (dailyFeatures.length > 0) {
                ...
                console.log(`📅 Updated period dates for daily features for ${userId}: ${dailyFeatures.map(f => f.feature_type).join(', ')}`);
            }
    ...
    async getFeatureUsage(userId, featureType) {
        ...
        try {
            ...
            if (subscription) {
                ...
                // Allow active status OR cancelled status (if period hasn't ended)
                if ((subscription.status === 'active' || subscription.status === 'cancelled') && endDate > now) {
    ...
    async incrementFeatureUsage(userId, featureType) {
        ...
        try {
            ...
            // Allow active status OR cancelled status (if period hasn't ended)
            let isSubscriptionActive = (subscription.status === 'active' || subscription.status === 'cancelled') && endDate > now;
        ...
        try {
            ...
            if (paymentData.user_email) {
                try {
                    ...
                    console.log('⚠️ Could not retrieve DataFast visitor ID:', error.message);
                }
            }
            ...
            case 'membership.renewed':
                ...
                if (userEmail && productId) {
                    ...
                    if (isRenewal && whopDates.validUntil) {
                        ...
                        console.log(`🔄 Processing RENEWAL for ${userEmail}, handling monthly reset`);
                        ...
                        console.log(`🆕 Processing NEW subscription for ${userEmail}`);
                        await this.activateSubscription(userEmail, planType, data.id, whopDates);
                    }
