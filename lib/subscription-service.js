 ...
 * period ends. This is not an edge case — it is the common path:
 *   - `order.paid` is an ORDER payload and carries no `current_period_end`,
 *     and it is one of our activation triggers.
 *   - one-time products (the weekly pass) never send a period end at all.
 * So this default is what actually lands in `subscription_ends_at` for most
 * activations. Getting it wrong silently grants the WRONG AMOUNT of access:
 * weekly used to fall through to the +1 month default, i.e. $8.99 bought a
 * month, and annual still granted 1 month instead of 1 year.
 * Lifetime is absent on purpose — it's special-cased to a null end date.
 */
...
export class SubscriptionService {
    ...
    async checkAccess(userId) {
        ...
        try {
            // Probe with hard errors surfaced FIRST: a transient DB failure must not
            // be mislabeled as "no subscription" and lock out a paying user.
            await this.getUserSubscription(userId, { throwOnError: true });

            // Covers owners and referral free-Pro grants — both resolve to 'pro'.
            const planType = await this.getUserPlanType(userId);
            if (planType && planType !== 'free' && planType !== 'none') return true;

            // plan_type didn't map to a paid tier — check the live "active
            // subscription" signal before blocking, so a record that just landed
            // from the payment webhook (or one whose plan_type the mapper doesn't
            // recognize) isn't turned away while Settings shows it as active.
            return await this.isSubscriptionActive(userId);
        } catch (error) {
            // Infra failure, not a verdict. The page-level paywall
            // (requirePaidSubscription) is the primary gate; this is a backstop.
            console.error('[checkAccess] check failed — failing open:', error?.message);
            return true;
        }
    }
