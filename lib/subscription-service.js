...
export const PLANS = {
    ...
    starter: {
        ...
        limits: {
            draft_reply: { limit: 50, period: 'daily' },
            schedule_call: { limit: 100, period: 'monthly' },
            ai_notes: { limit: 100, period: 'monthly' },
            sift_analysis: { limit: 50, period: 'daily' },
            arcus_ai: { limit: 200, period: 'daily' },
            email_summary: { limit: 100, period: 'daily' },
            openai_tokens: { limit: 1000000, period: 'monthly' }
        }
    },
    pro: {
        id: 'pro',
...
export class SubscriptionService {
    ...
    async getUserPlanType(userId) {
        ...
        // Expiration check - only if end date exists and status isn't active
        if (subscription.subscription_ends_at && subscription.status !== 'active') {
            const now = new Date();
            const endDate = new Date(subscription.subscription_ends_at);
    ...
    async getFeatureUsage(userId, featureType) {
        ...
        try {
            ...
            const totalLimit = featureLimits.limit + bonusLimit;
