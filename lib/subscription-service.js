...
export const PLANS = {
    ...
    lifetime: {
        ...
        price: 499,
        polarProductId: 'LIFETIME_PRODUCT_ID', // distinct Polar product (separate from pro); name/amount fallback classifies it
        polarPriceId: 'LIFETIME_PRICE_ID',
...
export class SubscriptionService {
    ...
    determinePlanType(data) {
        ...
        if (productId === PLANS.annual.polarProductId || priceId === PLANS.annual.polarPriceId) {
            return 'annual';
        }
                ...
                if (profile) {
                    ...
                    if (isNewSubscription && planType !== 'free' && !preferences.conversion_rewarded) {
                        try {
                            ...
                            if (inviterCode) {
                                console.log(`🚀 Conversion detected: ${userId} went Pro! Rewarding inviter ${inviterCode} with free Pro time`);

                                // Affiliate reward: grant the inviter ONE free month of Pro,
                                // stacking on top of any existing grant. Stored as
                                // preferences.free_pro_until and honored by getUserPlanType,
                                // so it elevates a free inviter to Pro without disturbing a
                                // real paid subscription.
                                const FREE_PRO_DAYS = 30;
                                ...
                                if (inviter) {
                                    ...
                                    const existingUntil = inviterPrefs.free_pro_until ? new Date(inviterPrefs.free_pro_until) : null;
                                    const base = (existingUntil && existingUntil > now) ? existingUntil : now;
                                    const newUntil = new Date(base.getTime() + FREE_PRO_DAYS * 24 * 60 * 60 * 1000);
                                    inviterPrefs.free_pro_until = newUntil.toISOString();
                                    inviterPrefs.plan = 'pro'; // reflect the grant in the cached flag
                                    ...
                                    console.log(`🎁 Granted ${FREE_PRO_DAYS} days free Pro to inviter ${inviter.user_id} (until ${inviterPrefs.free_pro_until})`);
                                }

                                preferences.conversion_rewarded = true; // Mark as done so we don't double-grant
                            }
                        } catch (convErr) {
