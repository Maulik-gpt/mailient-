...
class CalComAdapter extends BaseIntegrationAdapter {
  ...
  buildAuthUrl(state, baseUrl) {
    ...
    // DEMO BYPASS: If no client ID is provided, simulate a successful redirect immediately
    if (!process.env.CAL_COM_CLIENT_ID) {
      console.log(`[CalComAdapter] No Client ID found, using demo bypass`);
      return `${redirectUri}?code=MOCK_CAL_COM_CODE&state=${state || ''}`;
    }
  ...
  async exchangeCode(code, baseUrl) {
    // DEMO BYPASS
    if (code === 'MOCK_CAL_COM_CODE') {
      return {
        accessToken: 'mock_cal_com_token',
        refreshToken: 'mock_cal_com_refresh',
