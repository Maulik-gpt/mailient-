...
export class DatabaseService {
    ...
    try {
      ...
      if (!existingUser) {
        ...
        if (currentUser) {
          ...
          userBonus.arcus_ai = (userBonus.arcus_ai || 0) + welcomeAmount;
        ...
        if (referralCode) {
          ...
          if (inviter) {
            ...
            inviterBonus.arcus_ai = (inviterBonus.arcus_ai || 0) + 50;
  ...
  // ===== Arcus Missions: persistence layer =====
  ...
  // ===== Arcus Operator Runtime: persistence layer =====
  ...
  async createOperatorRun(userId, run) {
    try {
      ...
      if (error) {
        if (error.code === '42P01') {
          console.warn('arcus_runs table does not exist, skipping run persistence');
          return null;
        }
        throw error;
      }
  ...
  async updateOperatorRun(userId, runId, patch = {}) {
    try {
      ...
      if (error) {
        if (error.code === '42P01') {
          console.warn('arcus_runs table does not exist, skipping run update');
          return null;
        }
        throw error;
      }
  ...
  async getOperatorRunById(userId, runId) {
    try {
      ...
      if (error) {
        if (error.code === '42P01') {
          console.warn('arcus_runs table does not exist, cannot load run');
          return null;
        }
        throw error;
      }
  ...
  async getOperatorRunStepById(userId, runId, stepId) {
    try {
      ...
      if (error) {
        if (error.code === '42P01') {
          console.warn('arcus_run_steps table does not exist, cannot load run step');
          return null;
        }
        throw error;
      }
    ...
    try {
      ...
      if (error) {
        if (error.code === '42P01') {
          console.warn('arcus_run_steps table does not exist, skipping step persistence');
          return null;
        }
        throw error;
      }
  ...
  async updateOperatorRunStepStatus(userId, runId, stepId, status, detail = null, evidence = null) {
    try {
      ...
      if (error) {
        if (error.code === '42P01') {
          console.warn('arcus_run_steps table does not exist, skipping step status update');
          return null;
        }
        throw error;
      }
  ...
  async getOperatorRunSteps(userId, runId) {
    try {
      ...
      if (error) {
        if (error.code === '42P01') {
          console.warn('arcus_run_steps table does not exist, cannot load run steps');
          return [];
        }
        throw error;
      }
  ...
  async appendOperatorRunEvent(userId, runId, event) {
    try {
      ...
      if (error) {
        if (error.code === '42P01') {
          console.warn('arcus_run_events table does not exist, skipping run event persistence');
          return null;
        }
        throw error;
      }
  ...
  async getOperatorRunEvents(userId, runId, limit = 100) {
    try {
      ...
      if (error) {
        if (error.code === '42P01') {
          console.warn('arcus_run_events table does not exist, cannot load run events');
          return [];
        }
        throw error;
      }
  ...
  async createOperatorJob(userId, job) {
    try {
      ...
      if (error) {
        if (error.code === '42P01') {
          console.warn('arcus_jobs table does not exist, skipping job enqueue');
          return null;
        }
        throw error;
      }
  ...
  async claimOperatorJobs(maxJobs = 10, workerId = 'worker') {
    try {
      ...
      if (error) {
        if (error.code === '42P01') {
          console.warn('arcus_jobs table does not exist, no jobs to claim');
          return [];
        }
        throw error;
      }
  ...
  async completeOperatorJob(jobId, status = 'completed', errorMessage = null) {
    try {
      ...
      if (error) {
        if (error.code === '42P01') {
          console.warn('arcus_jobs table does not exist, skipping job completion update');
          return null;
        }
        throw error;
      }
  ...
  // ── Arcus chat session persistence ──────────────────────────────────────────

  /**
   * Upsert a full Arcus conversation snapshot.
   * Table: arcus_chat_sessions (id text PK, user_id text, messages jsonb, title text, updated_at timestamptz)
   * The table is auto-created if missing (run the migration separately via Supabase dashboard).
   */
  async saveArcusChatSession(userId, conversationId, messages, title) {
    try {
      const { error } = await this.supabase
      ...
      if (error) {
        if (error.code === '42P01') {
          console.warn('arcus_chat_sessions table missing — skipping DB save. Run migration to enable persistence.');
          return null;
        }
        throw error;
      }
      ...
      console.error('[saveArcusChatSession]', err.message);
      return null;
    }
  }

  async loadArcusChatSession(userId, conversationId) {
    try {
      const { data, error } = await this.supabase
        .from('arcus_chat_sessions')
        .select('*')
      ...
      console.error('[loadArcusChatSession]', err.message);
      return null;
    }
  }

  async listArcusChatSessions(userId, limit = 50) {
    try {
      const { data, error } = await this.supabase
        .from('arcus_chat_sessions')
      ...
      console.error('[listArcusChatSessions]', err.message);
      return [];
    }
  }
}
