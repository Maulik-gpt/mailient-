...
export class DatabaseService {
  ...
  // ===== Plan B: Email Forwarding (Inbound Parser) =====

  /**
   * Set a custom Mailient handle for a user
   */
  async setUserHandle(userId, handle) {
    ...
    try {
      const { data, error } = await this.supabase
        .from('user_profiles')
        .update({
          username: handle, // Re-using username field for the handle
          updated_at: new Date().toISOString()
        })
        .eq('user_id', userId);
  ...
  /**
   * Find a user ID by their Mailient handle
   */
  ...
  /**
   * Store an email received via forwarding webhook
   */
  async storeInboundEmail(userId, emailData) {
    try {
      const emailRecord = {
        user_id: userId,
        email_id: `inbound_${Date.now()}_${Math.random().toString(36).substr(2, 5)}`,
        thread_id: emailData.threadId || `thread_${Date.now()}`,
        subject: emailData.subject || '(No Subject)',
        from_email: emailData.from,
        to_email: emailData.to,
        date: new Date().toISOString(),
        snippet: emailData.text?.substring(0, 200) || '',
        body: emailData.text || '',
        html_body: emailData.html || '',
        labels: JSON.stringify(['inbound', 'unread']),
        created_at: new Date().toISOString()
      };

      const { data, error } = await this.supabase
        .from('user_emails')
        .upsert(emailRecord, { onConflict: 'user_id,email_id' });
