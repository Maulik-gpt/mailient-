/**
 * Arcus Tool Definitions — Phase 1: Agentic Loop
 *
...
// ─── Risk levels ────────────────────────────────────────────────────────────
// low    → read-only, safe to auto-execute
...
export const TOOL_DEFINITIONS = [
  ...
  {
    name: 'search_inbox',
    description: 'Search the user\'s Gmail inbox for emails matching a query. Returns subject, sender, date, and snippet for each result.',
    parameters: {
      query: { type: 'string', required: true, description: 'Gmail search query (e.g. "from:stripe subject:invoice newer_than:30d")' },
      maxResults: { type: 'number', required: false, default: 15, description: 'Maximum emails to return (1-50)' }
    },
    riskLevel: 'low',
  ...
  {
    name: 'read_email',
    description: 'Read the full content of a specific email by its message ID. Returns subject, from, to, date, and full body.',
    parameters: {
  ...
  {
    name: 'respond',
    description: 'Send a final conversational response to the user. Use this when you have all the information needed and want to present your answer. This ends the current agent loop.',
    parameters: {
    ...
    category: 'internal',
    idempotent: true,
    terminal: true  // signals the loop to stop
  }
];
