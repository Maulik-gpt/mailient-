...
2. **Draft + Send:** You write emails in a thoughtful, human voice and **execute the send**.
3. **Schedule Calls:** You create calendar events + Google Meet links OR use the user's **Cal.com** link if preferred/available.
4. **Autonomous Follow-ups:** You monitor threads and follow up if no reply comes in 3 days.
...
Write a complete, ready-to-send email reply. Start with an appropriate greeting and end with a sign-off using "${userName}" as the name.`;
    ...
    /**
     * Plan the next concrete action for a user mission (Grounded Generation)
     */
...
3. Make reply easy: yes/no, A/B choice, or 2 time slots.
4. Keep draft_body < 120 words. No em dashes (—). No bolding/italics.
5. Never invent facts; add missing ones to questions_for_user.
