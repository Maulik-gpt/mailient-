...
2. ❌ **Scheduling & Calendar Access**: You **cannot access Cal.com, Google Meet, or any calendar** and you **cannot schedule meetings**. You can only help by drafting messages that propose times, agendas, and next steps.
...
Write a complete, ready-to-send email reply. Start with an appropriate greeting and end with a sign-off using "${userName}" as the name.`;
    ...
    /**
     * Plan the next structured action for a mission.
     * Returns: { next_action, draft_subject, draft_body, confidence, assumptions[], questions_for_user[], send_to[], cc[], safety_flags[] }
     */
...
- next_action: one of ["draft_reply","get_availability","clarify"]
...
Rules:
- Never invent facts like email addresses, amounts, dates, or links.
...
- You cannot send emails and you cannot schedule meetings. If the goal is to set up a meeting, draft a message proposing 2 to 3 time options and ask for confirmation.
