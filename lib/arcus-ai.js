...
Write a complete, ready-to-send email reply. Start with an appropriate greeting and end with a sign-off using "${userName}" as the name.`;
    ...
    /**
     * Plan a mission: Break down a user goal into actionable steps
     */
    async planMission(goal, existingPlan = null) {
        try {
            const systemPrompt = `You are ARCUS. Your job is to break down a user request into a precise, step-by-step execution plan.
            
            AVAILABLE ACTIONS:
            - search_email: Find specific emails based on sender, subject, or keywords.
            - read_thread: Get the full content of a specific email thread.
            - draft_reply: Write a draft reply (but do NOT send).
            - schedule_check: Check calendar availability.
            - create_meeting: Create a calendar invite.
            - clarify: Ask the user for more information if the request is ambiguous.
            
            OUTPUT FORMAT:
            Return a JSON object with a "steps" array. Each step should be:
            {
                "actionType": "action_name",
                "label": "Human readable description",
                "description": "Technical details/query parameters"
            }
            
            EXAMPLE:
            User: "Find that email from Sarah about the Q3 report and draft a reply saying I'll review it by Friday."
            Plan:
            {
                "steps": [
                    { "actionType": "search_email", "label": "Search for emails from Sarah related to 'Q3 report'", "description": "from:Sarah \"Q3 report\"" },
                    { "actionType": "read_thread", "label": "Read the most relevant thread found", "description": "Get thread context" },
                    { "actionType": "draft_reply", "label": "Draft a reply confirming review by Friday", "description": "Say: I will review the Q3 report by this Friday." }
                ]
            }
            
            User: "Schedule a call with Mike tomorrow at 2pm."
            Plan:
            {
                "steps": [
                    { "actionType": "schedule_check", "label": "Check availability for tomorrow at 2pm", "description": "tomorrow 2pm duration:30" },
                    { "actionType": "create_meeting", "label": "Draft meeting invite for Mike", "description": "Title: Sync, Time: Tomorrow 2pm" }
                ]
            }
            
            Keep steps concise. If the request is simple (e.g. "Draft a reply to this"), just one step is fine.`;
            ...
            try {
                ...
                return {
                    steps: [
                        { actionType: 'clarify', label: 'Ask for clarification', description: 'Could not generate a plan.' }
                    ]
                };
            }
        } catch (error) {
            console.error('❌ Arcus planMission error:', error);
            throw error;
        }
    }
}
