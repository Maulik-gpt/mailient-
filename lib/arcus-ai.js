...
- **Full Task Execution**: You are no longer just a chatbot. You are a **Real Agent**. When a user approves a plan, YOU execute the code to Send Emails, Create Calendar Invites, and Search Records.
- **Never claim personal limitation**: Do not say "I cannot send emails" or "I don't have access to your calendar." You DO have access via the provided plan execution loop.
- **Precision**: If you are unsure about a time or recipient, ASK for clarification. Do not guess.
- **Reliability**: If your plan includes "Send Email", it WILL be sent via the Gmail API once approved.
- **Transparency**: Your internal `< thought > ` process is shown to the user. Be professional but detailed in your reasoning.
- **Sequential Flow**: Acknowledge the task clearly first. Say "I'm on it" or "Let me take care of that for you." Then present your plan.
...
3.  **Proactive Acknowledgment**: When a task is requested, determine its viability and say "I'm on it" or "Starting on that now." Acknowledge the mission before diving into details.
...
Write a complete, ready-to-send email reply. Start with an appropriate greeting and end with a sign-off using "${userName}" as the name.`;
    ...
    parseDraftIntent(message) {
        ...
        // Check for draft/reply patterns (Precedence: Compose > Reply)
        const composePatterns = [
            /(?:compose|write|draft|send)\s+(?:a\s+)?(?:new\s+)?email\s+to\s+([^\s,]+@[^\s,]+\.[^\s,]+|[A-Z][a-z]+(?:\s+[A-Z][a-z]+)?)/i,
            /email\s+to\s+([^\s,]+@[^\s,]+\.[^\s,]+|[A-Z][a-z]+(?:\s+[A-Z][a-z]+)?)/i,
            /message\s+([^\s,]+@[^\s,]+\.[^\s,]+|[A-Z][a-z]+(?:\s+[A-Z][a-z]+)?)/i
        ];
        ...
        // HEURISTIC: If "him/her/them/it" is used, look for an email/name previously mentioned in the message
        if (result.isDraftRequest && (result.recipient === 'him' || result.recipient === 'her' || result.recipient === 'them' || result.recipient === 'it')) {
...
The JSON schema is:
{
  ...
  "goal": "Professional 1-sentence goal",
  "steps": ["Step 1: Cognitive analysis of...", "Step 2: Execution of...", ...],
  "tools": ["email_search", "email_read", "send_email", "create_draft", "calendar_availability", "create_meeting", "schedule_check"],
  "draft_preview": { "to": ["recipient@example.com"], "cc": [], "subject": "", "body": "" } | null,
  "invite_preview": { "title": "", "attendees": ["email1@ex.com"], "slot": "suggested time", "duration": "30", "location": "Google Meet" } | null,
  "risk_flags": [],
  "confidence": 1.0,
  "assumptions": [],
  "questions_for_user": [],
...
Notion AI Protocol:
- If a user mentions an email address and an action (like draft or schedule), LINK THEM. 
- "him/her" usually refers to the email address mentioned in the same message.
- "tools" should include EVERYTHING needed for the mission.
- If you see "Cal.com", include create_meeting.
- If you see "draft", include create_draft.
- If you see "send", include send_email.
- Propose a plan card for ANY multi-step request.
- Be extremely precise with recipients and times.
...
${conversationHistory.length > 0 ? `Recent conversation:\n${conversationHistory.slice(-6).map(m => `[${m.role}]: ${m.content.substring(0, 150)}`).join('\n')}\n` : ''}`;
            ...
            const { content: rawContent, thought } = this.extractResponse(response);
            ...
            return {
                ...
                thought: thought // Return the internal reasoning too
            };
        } catch (error) {
