...
export class ArcusAIService {
    ...
    async callOpenRouter(messages, options = {}) {
        // MAILIENT POLICY: Arcus prefers Trinity Large Preview but has reliable fallbacks
        const models = [
...
5. **Schedule Calls:** You create calendar events + Google Meet links OR use the user's Cal.com link if preferred/available.
...
## Structured Output (Strict JSON):
{
  ...
  "safety_flags": ["one or more of: new_recipient, external_domain, mentions_money, mentions_legal, mentions_medical, attachment_forwarding, large_recipient_list"]
        ...
        const messages = [
            { role: 'system', content: systemPrompt },
            {
                ...
                content: JSON.stringify({
                    ...
                    last_messages: lastMessages.slice(-3),
                    extracted_facts: extractedFacts,
                    user_preferences: userPreferences,
                    recent_conversation: conversationHistory.slice(-5).map(m => ({ role: m.role, content: m.content?.substring(0, 200) }))
                })
            }
        ];
    ...
    /**
     * Generate internalized thoughts for the Thinking indicator
     */
    async generateThoughts(missionGoal, context = {}) {
        const systemPrompt = `You are ARCUS, a high-performance email agent. 
Briefly state your immediate technical thoughts for: "${missionGoal}".
Rules:
1. Max 20 words.
2. Use active verbs: "Locating threads", "Parsing subject lines", "Comparing availability".
3. No filler ("I will", "Now I am").
4. Focus on the 'why' and 'how'.`;
        ...
        try {
            ...
            return "Analyzing goal and preparing execution steps.";
        }
    }
...
- get_availability: check calendar/cal.com for slots
