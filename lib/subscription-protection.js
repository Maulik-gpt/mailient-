...
export async function assertPaidAccess(email) {
    ...
    try {
        // Probe the subscription table with hard errors surfaced FIRST. If the DB
        // read genuinely fails this throws → we fail OPEN in the catch instead of
        // mislabeling a paying user as free. (getUserPlanType below uses the lenient
        // null-on-error path, but it only runs once this probe proves the DB is up.)
        await subscriptionService.getUserSubscription(email, { throwOnError: true });
