...
export class ArcusMissionService {
  ...
  async executeMission(mission, context = {}) {
        ...
        switch (currentStep.actionType) {
          ...
          case 'draft_reply': {
            ...
            const planCard = {
              ...
              subject: aiPlan.draft_subject || 'Re: your email',
            ...
            // If confidence is high and no questions, we could auto-execute, 
            // but the spec says "confirms" at the end. 
            // For now, drafting is the "execution" for many missions.
            mission.status = 'waiting_on_user'; // Stop here to show the draft
            stepResult = planCard;
            break;
          }
    ...
    if (!currentStep && mission.status !== 'failed') {
      mission.status = 'done';
    }
