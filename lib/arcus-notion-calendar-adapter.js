...
class NotionCalendarAdapter extends BaseIntegrationAdapter {
  ...
  buildAuthUrl(state, baseUrl) {
    ...
    if (!clientId) {
      console.log(`[NotionCalendarAdapter] No Client ID found, using demo bypass`);
      return `${redirectUri}?code=MOCK_NOTION_CAL_CODE&state=${state || ''}`;
    }
  ...
  async exchangeCode(code, baseUrl) {
    // DEMO BYPASS
    if (code === 'MOCK_NOTION_CAL_CODE') {
      return {
        accessToken: 'mock_notion_cal_token',
        refreshToken: null,
        expiresAt: undefined,
