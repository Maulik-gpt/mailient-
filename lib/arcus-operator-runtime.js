...
export class ArcusOperatorRuntime {
  ...
  buildExecutionPolicy(canvasType = 'none', requiresApproval = false, runId = '', taskType = null) {
    ...
    return {
      ...
      autoExecuteActions: actions
        .filter((a) => a.autoExecute && !a.requiresApproval)
        .map((a) => a.actionType),
      approvalTokens
    };
  }
  ...
  async initializeRun({
    ...
    if (providedRunId) {
      ...
      if (existingRun) {
        ...
        return {
          ...
          executionPolicy: existingRun?.memory?.executionPolicy || null
        };
      }
    }
    ...
    const executionPolicy = this.buildExecutionPolicy(
      intentAnalysis?.canvasType || canvasType || 'none',
      requiresApproval,
      runId,
      effectiveTaskType
    );
    ...
    return { run, plan, requiresApproval, taskType: effectiveTaskType, executionPolicy };
  }
  ...
  requiresApprovalForAction(actionType, executionPolicy = null) {
    ...
    const effectivePolicy = executionPolicy || null;
    const policyAction = effectivePolicy?.actions?.find((a) => a.actionType === actionType);
    if (policyAction) return Boolean(policyAction.requiresApproval);
    if (effectivePolicy?.requiresApproval && isCriticalAction(actionType)) return true;
    return requiresApprovalForActionFromPolicy(actionType, effectivePolicy);
  }
  ...
  async validateApprovalToken(runId, actionType, approvalToken) {
    ...
    const memory = run.memory || {};
    const policy = memory.executionPolicy || null;
    const requiresApproval = this.requiresApprovalForAction(actionType, policy);
    if (!requiresApproval) {
      return { ok: true, reason: 'approval_not_required', run };
    }

    const expected = memory.approvalTokens?.[actionType];
    const consumed = memory.consumedApprovalTokens?.[actionType];

    if (!expected) return { ok: false, reason: 'approval_token_missing' };
