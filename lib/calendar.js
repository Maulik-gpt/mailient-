...
export class CalendarService {
    ...
    /**
     * List events for the primary calendar
     */
...
CalendarService.prototype.createEvent = function (opts = {}) {
    // Translate from CanvasActionHandlers { title, description, date, time, duration }
    // To CalendarService { summary, description, startTime, endTime }
    const start = new Date(`${opts.date}T${opts.time}`);
    const end = new Date(start.getTime() + (opts.duration || 30) * 60000);
...
CalendarService.prototype.getAvailability = function (opts = {}) {
    // Translate { date, duration } to getBusySlots(timeMin, timeMax)
    const startTime = new Date(`${opts.date}T00:00:00Z`);
    const endTime = new Date(startTime.getTime() + 24 * 60 * 60 * 1000);
    return this.getBusySlots(startTime.toISOString(), endTime.toISOString());
};
