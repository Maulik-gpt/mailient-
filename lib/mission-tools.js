...
export class MissionTools {
  ...
  async getAvailability({ attendees = [], window, duration = 30, timezone = 'UTC' }) {
    ...
    try {
      ...
      // If no Cal.com slots found or failed, use Google Calendar
      if (suggestedSlots.length === 0 && calendar) {
        ...
        if (current < now) current = new Date(now.getTime() + 60 * 60000);
        ...
        while (suggestedSlots.length < 3 && current < new Date(timeMax)) {
          ...
          if (!isBusy) {
            ...
            current = new Date(current.getTime() + 120 * 60000);
          } else {
  ...
  async createMeeting({ title, attendees = [], slot, notes, location = 'google_meet' }) {
    ...
    return {
      event_id: result.id,
      meet_link: result.hangoutLink || result.zoomLink || result.htmlLink,
