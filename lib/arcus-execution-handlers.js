...
export class ExecutionHandlers {
  ...
  async undo(actionType, params, result, context) {
    console.log(`🧪 [Arcus Telemetry] Executing compensation for ${actionType}`);
    ...
    try {
      ...
      console.error(`❌ [Arcus Telemetry] Undo failed for ${actionType}:`, err);
  ...
  async handleDeleteCalendarEvent(payload, context) {
    ...
    const { googleFetch } = await import('./arcus/tools/http-tokens');
