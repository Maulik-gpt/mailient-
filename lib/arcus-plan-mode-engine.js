...
export class PlanModeEngine {
  ...
  _buildPlanGenerationPrompt(context) {
    return `You are ARCUS Plan Engine. Generate ULTRA-DETAILED, structured execution plans for an email workspace.
...
4. CONTEXTUAL: Reference specific emails, names, dates from the user's request.
...
Return ONLY valid JSON. No markdown wrapping, no explanation outside the JSON.`;
  ...
  async _generateFallbackPlan({ message, runId, conversationId, intent, complexity }) {
    ...
    const plan = {
      ...
      objective: `Arcus is planning how to handle your request: "${message}"\n\nThe AI planning engine will analyze your objective and determine the most effective approach. This may include searching your emails, drafting responses, scheduling meetings, or creating documents based on what your request requires.`,
      status: 'draft',
      version: 1,
      locked: false,
      intent,
