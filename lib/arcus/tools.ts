...
export const TOOL_SCHEMAS: ToolSchema[] = [
  ...
  },
  {
    ...
    description:
      'Create a cold-outreach CAMPAIGN: many recipients, one personalized email each, drafted in the user\'s voice and sent PACED over days (never a blast). ' +
      'Use this whenever the user wants to email many people (a list, a CSV, a Notion database) — NEVER loop send_email or gmail_batch_send_emails for cold outreach. ' +
      'Parse the recipients yourself first (from pasted text, an attached CSV, or notion_query_database rows) into the recipients array — every context column you pass makes the personalization sharper. ' +
      'This tool only creates + starts DRAFTING. Nothing can send until the user reviews the drafts and explicitly approves the campaign in the review screen. ' +
      'Pacing: ~40/day cap (adjustable 10-100), business-hours sending, minutes between sends, reputation ramp-up. Recipients contacted in the last 30 days or on the suppression list are auto-excluded. ' +
      'Output: campaign summary + canvasData campaign card. ' +
      'Errors (success:false): validation_error (bad/empty recipients or thin brief), campaign_create_failed.',
    input_schema: {
      type: 'object',
      properties: {
        name: { type: 'string', description: 'Short campaign name, e.g. "Dental clinics — March".' },
        brief: { type: 'string', description: 'The pitch and goal IN DETAIL: who the sender is, what they\'re offering, why these recipients, what a good personalized hook looks like, and the desired reply/CTA. The drafts are only as good as this brief.' },
        recipients: {
          type: 'array',
          description: 'One entry per person. Include every known field — context drives personalization.',
          items: {
            type: 'object',
            properties: {
              email: { type: 'string', description: 'Recipient email (required).' },
              name: { type: 'string', description: 'Their name, if known.' },
              company: { type: 'string', description: 'Company/organization, if known.' },
              context: { type: 'object', description: 'Any other columns from the source (role, city, niche, notes…) as key/value pairs.' },
            },
            required: ['email'],
          },
        },
        daily_cap: { type: 'number', description: 'Max sends per day (10-100). Default 40.' },
        research_depth: { type: 'string', enum: ['light', 'standard', 'deep'], description: 'How much per-recipient research to do before drafting. Default standard (fetches their company site).' },
        subject_hint: { type: 'string', description: 'Optional direction for subject lines.' },
      },
      required: ['name', 'brief', 'recipients'],
    },
  },
  {
    name: 'get_campaign_status',
    description:
      'Get the live status of an outreach campaign: funnel counts (drafted/sent/replied/meetings/failed), pacing, domain health, and any error. ' +
      'Resolve by id or by (partial) name. Use when the user asks how a campaign is going. ' +
      'Output: status summary + refreshed canvasData campaign card. ' +
      'Errors (success:false): not_found.',
    input_schema: {
      type: 'object',
      properties: {
        campaignId: { type: 'string', description: 'Campaign id, if known.' },
        name: { type: 'string', description: 'Campaign name (partial ok) — used when id is unknown.' },
      },
    },
  },
  {
    ...
    description:
      'Pause, resume, or cancel an outreach campaign. Pause stops future sends and cancels queued-but-unsent emails; resume continues (only if the user previously approved the campaign — this can NEVER start an unapproved campaign); cancel is terminal. ' +
      'Errors (success:false): not_found, invalid_state.',
    input_schema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description:
      'List the replies a campaign has received, with the classified intent (interested/question/objection/not_now) and reply snippets. Use when the user asks "any replies on my campaign?". ' +
      'Errors (success:false): not_found.',
    input_schema: {
      type: 'object',
      properties: {
        campaignId: { type: 'string', description: 'Campaign id, if known.' },
        name: { type: 'string', description: 'Campaign name (partial ok).' },
      },
    },
  },
  {
    ...
    description:
      'Read rows from ANY Notion database (contacts, leads, CRM…) as flat records — every property becomes a key/value. ' +
      'Resolve by databaseId or by a name hint. Ideal for pulling a contact list to feed create_outreach_campaign. ' +
      'Output: numbered rows with all non-empty properties. ' +
      'Errors (success:false): notion_not_connected, not_found, upstream_notion.',
    input_schema: {
      type: 'object',
      properties: {
        databaseId: { type: 'string', description: 'Notion database id, if known.' },
        database: { type: 'string', description: 'Database name hint, e.g. "Leads" — resolved via search.' },
        pageSize: { type: 'number', description: 'Max rows (default 50, max 100).' },
      },
    },
  },
  {
...
export async function executeTool(
  ...
  try {
    ...
    switch (name) {
      ...
      case 'create_outreach_campaign': result = await createOutreachCampaignTool(userId, input); break;
      case 'get_campaign_status':      result = await getCampaignStatusTool(userId, input); break;
      case 'set_campaign_state':       result = await setCampaignStateTool(userId, input); break;
      case 'list_campaign_replies':    result = await listCampaignRepliesTool(userId, input); break;
      case 'notion_query_database':    result = await notionQueryDatabase(userId, input); break;
...
// ─── Outreach campaigns (Arcus Outreach) ──────────────────────────────────────
//
// The engine lives in lib/arcus/outreach.ts. These executors are thin: they
// validate, delegate, and shape the campaign card. Approval is deliberately
// NOT a tool — the review screen's Approve button is the only path to
// 'sending', so the model can never start a send.

/** Flatten any Notion property value to a display string (best-effort). */
function notionPropToString(prop: any): string {
...
async function notionQueryDatabase(userId: string, input: any): Promise<ToolResult> {
  const token = await getNotionToken(userId);
  if (!token) return failureResult('Notion is not connected.', 'notion_not_connected');
  const headers: Record<string, string> = {
    Authorization: `Bearer ${token}`,
  ...
  if (!dbId) {
    if (!dbHint) return failureResult('Provide databaseId or a database name hint.', 'validation_error');
    try {
      const res = await fetch('https://api.notion.com/v1/search', {
        method: 'POST',
        headers,
        body: JSON.stringify({ query: dbHint, filter: { value: 'database', property: 'object' }, page_size: 10 }),
        signal: AbortSignal.timeout(10000),
      });
      if (res.ok) {
        const data = await res.json();
        ...
        if (best && best.score > 0) {
          dbId = best.r.id;
          resolvedTitle = best.r.title?.[0]?.plain_text || dbHint;
        }
      }
    } catch { /* falls through to not_found */ }
    if (!dbId) return failureResult(`No Notion database matching "${dbHint}" found.`, 'not_found');
  }
  ...
  const qData = await qRes.json();
  const rows: any[] = qData.results || [];
  if (!rows.length) return { output: `"${resolvedTitle}" has no rows.` };

  const lines = rows.map((row: any, i: number) => {
    const fields = Object.entries(row.properties || {})
      .map(([key, prop]) => ({ key, value: notionPropToString(prop) }))
      .filter(f => f.value)
      .map(f => `${f.key}: ${f.value}`);
    return `${i + 1}. ${fields.join(' | ') || '(empty row)'}`;
  });
  return {
    output:
      `${rows.length} row${rows.length === 1 ? '' : 's'} from "${resolvedTitle}"${qData.has_more ? ' (more exist — raise pageSize)' : ''}:\n\n` +
      lines.join('\n'),
  };
}

/** Resolve a campaign by explicit id or (partial) name. */
async function resolveCampaign(supabase: any, userId: string, input: any): Promise<any | null> {
  const id = (input.campaignId || '').trim();
  if (id) {
    const { data } = await supabase
      .from('arcus_campaigns').select('*').eq('id', id).eq('user_id', userId).maybeSingle();
    return data || null;
  }
  const name = (input.name || '').trim();
  if (!name) {
    // No identifier at all — most recent campaign is the sensible default.
    const { data } = await supabase
      .from('arcus_campaigns').select('*').eq('user_id', userId)
...
function campaignCardData(snapshot: any): NonNullable<ToolResult['canvasData']> {
  const counts = snapshot.counts || {};
  return {
    title: `Outreach: ${snapshot.name}`,
    type: 'campaign',
    markdown: [
      `## ${snapshot.name}`,
      '',
      `**Status:** ${snapshot.status}`,
      `${counts.recipients ?? 0} recipients · ${counts.drafted ?? 0} drafted · ${counts.sent ?? 0} sent · ${counts.replied ?? 0} replied`,
      '',
      'Open **Outreach** in the dashboard to review drafts and approve the campaign.',
    ].join('\n'),
    pageMeta: { campaign: snapshot },
  };
}

function describeCampaign(s: any): string {
  const c = s.counts;
  const lines = [
    `**${s.name}** — ${s.status}`,
    `${c.recipients} recipients · ${c.drafted} drafted · ${c.sent} sent · ${c.replied} replied${c.meeting ? ` · ${c.meeting} meetings` : ''}${c.failed ? ` · ${c.failed} failed` : ''}${c.excluded ? ` · ${c.excluded} excluded` : ''}`,
  ];
  if (s.status === 'drafting') lines.push('Drafting continues in the background — the review screen fills as drafts land.');
  if (s.status === 'review') lines.push('All drafts are ready. Nothing sends until they approve the campaign in the review screen (Outreach → review).');
  if (s.status === 'sending') lines.push(`Sending paced at up to ${s.effectiveCapToday}/day (cap ${s.dailyCap}), business hours, minutes apart.`);
  if (s.domainHealth?.advice) lines.push(`⚠ Deliverability: ${s.domainHealth.advice}`);
  if (s.lastError) lines.push(`Last error: ${s.lastError}`);
  return lines.join('\n');
}

async function createOutreachCampaignTool(userId: string, input: any): Promise<ToolResult> {
  try {
    const { getSupabaseAdmin } = await import('../supabase.js');
    const supabase = getSupabaseAdmin();
    const { createCampaign, draftCampaignBatch, getCampaignSnapshot } = await import('./outreach');

    const created = await createCampaign(supabase, userId, {
      name: input.name,
      brief: input.brief,
      recipients: Array.isArray(input.recipients) ? input.recipients : [],
      dailyCap: input.daily_cap,
      researchDepth: input.research_depth,
      subjectHint: input.subject_hint,
    });
    if (!created.ok || !created.campaignId) {
      return failureResult(created.error || 'Could not create the campaign.', 'campaign_create_failed');
    }

    // Kick the first drafting batch inline so the user sees real progress
    // immediately; the cron continues the rest.
    await draftCampaignBatch(supabase, userId, created.campaignId);
    const snapshot = await getCampaignSnapshot(supabase, userId, created.campaignId);

    const excludedNote = created.excluded && created.excluded.length
      ? `\nExcluded ${created.excluded.length}: ${created.excluded.slice(0, 5).map(e => `${e.email} (${e.reason})`).join(', ')}${created.excluded.length > 5 ? '…' : ''}`
      : '';
    const healthNote = created.domainHealth?.advice ? `\n⚠ ${created.domainHealth.advice}` : '';

    return {
      output:
        `Campaign "${input.name}" created with ${created.accepted} recipients — drafting has started and continues in the background.${excludedNote}${healthNote}\n\n` +
        `Nothing will send until the user reviews the drafts and approves the campaign. Tell them the drafts are being written and where to review (the campaign card links to the review screen).`,
      canvasData: snapshot ? campaignCardData(snapshot) : undefined,
    };
  } catch (e: any) {
    return failureResult(`Campaign creation failed: ${e?.message || 'unknown error'}`, 'campaign_create_failed');
  }
}

async function getCampaignStatusTool(userId: string, input: any): Promise<ToolResult> {
  try {
    const { getSupabaseAdmin } = await import('../supabase.js');
    const supabase = getSupabaseAdmin();
    const { getCampaignSnapshot } = await import('./outreach');
    const campaign = await resolveCampaign(supabase, userId, input);
    if (!campaign) return failureResult('No matching campaign found.', 'not_found');
    const snapshot = await getCampaignSnapshot(supabase, userId, campaign.id);
    if (!snapshot) return failureResult('No matching campaign found.', 'not_found');
    return { output: describeCampaign(snapshot), canvasData: campaignCardData(snapshot) };
  } catch (e: any) {
    return failureResult(`Could not read campaign status: ${e?.message || 'unknown error'}`, 'not_found');
  }
}

async function setCampaignStateTool(userId: string, input: any): Promise<ToolResult> {
  const action = (input.action || '').trim();
  if (!['pause', 'resume', 'cancel'].includes(action)) {
    return failureResult('action must be pause, resume, or cancel.', 'validation_error');
  }
  try {
    const { getSupabaseAdmin } = await import('../supabase.js');
    const supabase = getSupabaseAdmin();
    const { setCampaignState, getCampaignSnapshot } = await import('./outreach');
    const campaign = await resolveCampaign(supabase, userId, input);
    if (!campaign) return failureResult('No matching campaign found.', 'not_found');
    const res = await setCampaignState(supabase, userId, campaign.id, action as any);
    if (!res.ok) return failureResult(res.error || 'State change failed.', 'invalid_state');
    const snapshot = await getCampaignSnapshot(supabase, userId, campaign.id);
    return {
      output: `Campaign "${campaign.name}" is now ${res.status}.` +
        (action === 'pause' ? ' Queued-but-unsent emails were cancelled; approved drafts are kept and resume exactly where they left off.' : ''),
      canvasData: snapshot ? campaignCardData(snapshot) : undefined,
    };
  } catch (e: any) {
    return failureResult(`State change failed: ${e?.message || 'unknown error'}`, 'invalid_state');
  }
}

async function listCampaignRepliesTool(userId: string, input: any): Promise<ToolResult> {
  try {
    const { getSupabaseAdmin } = await import('../supabase.js');
    const supabase = getSupabaseAdmin();
    const campaign = await resolveCampaign(supabase, userId, input);
    ...
    const { data: replies } = await supabase
      ...
      .select('email, name, reply_intent, reply_snippet, replied_at, status')
      .eq('campaign_id', campaign.id)
      .in('status', ['replied', 'meeting'])
      .order('replied_at', { ascending: false })
      .limit(25);
    if (!replies?.length) {
      return { output: `No replies yet on "${campaign.name}" (${campaign.sent_count || 0} sent so far).` };
    }
    const lines = replies.map((r: any, i: number) =>
      `${i + 1}. ${r.name || r.email}${r.reply_intent ? ` — **${r.reply_intent}**` : ''}${r.reply_snippet ? `: "${String(r.reply_snippet).slice(0, 140)}"` : ''}`,
    );
    return { output: `${replies.length} repl${replies.length === 1 ? 'y' : 'ies'} on "${campaign.name}":\n\n${lines.join('\n')}` };
  } catch (e: any) {
    return failureResult(`Could not list replies: ${e?.message || 'unknown error'}`, 'not_found');
  }
}
