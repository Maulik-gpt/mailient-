...
`;
    ...
    async generateInboxIntelligence(gmailData, privacyMode = false) {
        // Increase pool to 80 emails and sample more evenly (top 40, then 40 from the rest)
        const topEmails = gmailData.slice(0, 40);
        const restEmails = gmailData.slice(40);
        const sampledRest = restEmails.filter((_, i) => i % Math.max(1, Math.floor(restEmails.length / 40)) === 0).slice(0, 40);
        const emails = [...topEmails, ...sampledRest].slice(0, 80);
        ...
        console.log(`🚀 [TurboSift] Analyzing ${emails.length} sampled emails for high-fidelity intelligence...`);
        
        // Split into 4 parallel batches for thoroughness
        const batchSize = Math.ceil(emails.length / 4);
        const batches = [];
        ...
        const runBatch = async (batchEmails, batchIndex, offset) => {
            const emailList = batchEmails.map((e, idx) =>
                `ID: ${idx + 1 + offset} | From: ${e.from} | Subject: ${e.subject} | Content: ${(e.body || e.snippet || '').substring(0, 600)}\n`
            ).join('\n---\n');

            const prompt = `You are a high-level Strategic Intelligence Analyst. Analyze these emails and identify key business signals.${privacyMode ? '\nPRIVACY MODE ENABLED.' : ''}
            
            OUTPUT JSON SCHEMA:
            {
                "opportunity": { "title": "Strategic Opportunities", "summary": "One-liner", "description": "Detailed strategic context (25-40 words)", "relevance": 1-10, "ids": [IDs] },
                "urgent": { "title": "Critical / Urgent", "summary": "One-liner", "description": "Why this is urgent and what is blocked (25-40 words)", "relevance": 1-10, "ids": [IDs] },
                "lead": { "title": "Growth & Leads", "summary": "One-liner", "description": "Lead quality and potential value (25-40 words)", "relevance": 1-10, "ids": [IDs] },
                "risk": { "title": "Operational Risks", "summary": "One-liner", "description": "Nature of risk and potential fallout (25-40 words)", "relevance": 1-10, "ids": [IDs] },
                "follow_up": { "title": "Pending Follow-ups", "summary": "One-liner", "description": "Status of conversation and next move (25-40 words)", "relevance": 1-10, "ids": [IDs] },
                "important": { "title": "Key Updates", "summary": "One-liner", "description": "Significance of this update (25-40 words)", "relevance": 1-10, "ids": [IDs] }
            }

            CRITICAL: 
            1. Descriptions MUST be insightful and specific to the data. No generic filler.
            2. Use the provided numerical ID from the label.
            3. If no clear items exist for a category, return empty arrays/null.
            ...
            try {
                // Use the more capable Nemotron-3 model for deeper analysis
                ...
                console.error(`❌ Sift Batch ${batchIndex} failed:`, error.message);
                return {};
            }
            ...
            categories.forEach(cat => {
                if (batchRes[cat]) {
                    ...
                    if (res.summary && res.summary.length > 10) {
                        ...
                        // Merge descriptions
                        if (res.description) {
                            if (!result[cat].description) {
                                result[cat].description = res.description;
                            } else if (!result[cat].description.includes(res.description)) {
                                result[cat].description += ' ' + res.description;
                            }
                        }
                    }
