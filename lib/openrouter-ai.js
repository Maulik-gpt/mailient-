...
export class OpenRouterAIService {
    ...
    // Premium-first pass, mirroring lib/arcus/engine.ts. When ALLOW_PAID_MODELS /
    // ARCUS_PREMIUM_MODE is set, drafts (and every call here) try paid models
    // FIRST — so the draft path stops failing with "model side" errors once free
    // models are exhausted. Read at call time so env changes apply without rebuild.
    static premiumModels() {
        if (process.env.ARCUS_PREMIUM_MODE !== 'true' && process.env.ALLOW_PAID_MODELS !== 'true') return [];
    ...
    async callOpenRouter(messages, options = {}) {
        const models = options.singleModel
            ...
            : [
                ...OpenRouterAIService.premiumModels(),
                options.model || this.model,
                ...DEFAULT_AI_MODELS
        ...
        // Allow up to 4 attempts by default, configurable via options
        let totalAttempts = 0;
    ...
    async callOpenRouterStreamWithFallback(messages, options = {}) {
        ...
        let lastError = null;
        // Try up to 6 distinct models before giving up
        const MAX_STREAM_ATTEMPTS = Math.min(models.length, 6);

        for (let i = 0; i < MAX_STREAM_ATTEMPTS; i++) {
            const modelId = models[i];
            try {
                console.log(`📡 [OpenRouter] Stream attempt ${i + 1}/${MAX_STREAM_ATTEMPTS} with ${modelId}`);
    ...
    async callOpenRouterStream(messages, options = {}) {
        ...
        for (let keyAttempt = 0; keyAttempt < keys.length; keyAttempt++) {
            ...
            if (OpenRouterAIService.blockedKeys.has(apiKey)) {
                ...
                if (!foundUnblocked) {
                    console.warn('🛑 [callOpenRouterStream] All available keys are currently daily rate-limited.');
                    throw new Error('429: All keys daily rate-limited');
                }
            }
