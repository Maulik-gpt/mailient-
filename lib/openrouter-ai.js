...
export class OpenRouterAIService {
        ...
        try {
            ...
            return await Promise.race([
                (async () => {
                    ...
                    console.log(`📡 [Sift AI] Analysis started for ${totalEmails.length} emails in ${batches.length} batches (sequential)...`);
                    ...
                    for (let i = 0; i < batches.length; i++) {
                        ...
                        while (retries <= MAX_RETRIES) {
                            try {
                                result = await this._processIntelligenceBatch(batches[i], batches[i].map(e => e.id), categories, privacyMode);
                                if (result && result.length > 0) break; // Success
                                
                                // If result is empty, it might be a temporary model failure
                                console.warn(`⚠️ [Sift AI] Batch ${i + 1} returned empty results, retrying... (${retries + 1}/${MAX_RETRIES})`);
                            } catch (err) {
                    ...
                    batchResults.forEach(batchResult => {
                        ...
                        // batchResult is now an array of { id, category, reason }
                        batchResult.forEach(item => {
                            ...
                            if (categories.includes(cat) && item.id) {
                                ...
                                // Accumulate reasons for description
                                if (item.reason && !mergedResult[cat].description.includes(item.reason)) {
                                    mergedResult[cat].description = mergedResult[cat].description 
                                        ? mergedResult[cat].description + " " + item.reason 
                                        : item.reason;
                                }
                                
                                // Set a basic summary if not present
                                if (!mergedResult[cat].summary) {
                                    mergedResult[cat].summary = `Detected ${cat} items.`;
                                }
            ...
            // Return empty structure instead of throwing to prevent 500 errors
            const fallbackResult = this.createFallbackResponse(categories);
            return this.buildEnhancedResponse(fallbackResult, gmailData.map(e => e.id), categories);
        }
    }
    ...
    async _processIntelligenceBatch(emails, emailIds, categories, privacyMode) {
        ...
        const systemPrompt = `You are an email classifier. Categorize each email into exactly ONE of these categories: ${categories.join(', ')}.

OUTPUT FORMAT (ONE LINE PER EMAIL):
ID: <id> | CATEGORY: <category> | REASON: <1-sentence reason>

CATEGORIES:
- opportunity: Partnerships, sales, business deals.
- urgent: Deadlines, emergencies, time-sensitive.
- lead: New customer inquiries, pricing, demo requests.
- risk: Complaints, cancellations, technical issues.
- follow_up: Replies needed, check-ins, meetings.
- important: Confirmations, security, general priority.

STRICT RULE: Only output the formatted lines. No conversational text.`;
        ...
        try {
            ...
            const lines = rawContent.split('\n').filter(l => l.includes('ID:') && l.includes('CATEGORY:'));
            
            const results = lines.map(line => {
                const idMatch = line.match(/ID:\s*([a-zA-Z0-9_:.-]+)/i);
                const catMatch = line.match(/CATEGORY:\s*([a-z_]+)/i);
