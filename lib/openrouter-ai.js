...
1. DEEP THINKING: First, deeply consider ${fromName}'s identity and intent. 
   - Is this an automated system, platform notification (e.g., Devpost, LinkedIn), newsletter, or no-reply address? 
   - If it is a bot or marketing blast, recognize that an informal conversational reply (like "thanks for the update!") is naive and inappropriate.
   - Evaluate what ${userName} (given their role) would *actually* want to say here, if anything.
   - Put your entire reasoning process inside <think> and </think> tags.
2. OUTPUT ONLY THE FINAL EMAIL TEXT inside <email> and </email> tags. No meta-commentary outside of <think> tags.
3. ADAPT STYLE TO RECIPIENT: If the recipient is a bot/system, use a more functional or professional tone, even if the user profile is casual. Do not be conversational with machines.
4. NO EMOJIS TO BOTS: Do not use emojis when drafting replies to automated systems, platforms, or newsletters.
5. FORBIDDEN: Do not invent a corporate tone if the profile is casual for real humans. Match the profile perfectly for human-to-human interaction.
6. You MUST strictly use ONLY the precise greetings and sign-offs explicitly listed in the voice profile. Do NOT invent new ones.
7. If the voice profile indicates lowercase-heavy or omitted periods, you MUST apply that exact formatting.
8. Address the context of the email clearly and efficiently.
9. Maximum ${tone === 'concise' ? '50' : '150'} words. No em dashes (use commas/periods).

OUTPUT FORMAT (Must be exactly this structure):
<think>Your analysis of the sender (Real person vs System) and the specific dynamics...</think>
<email>hey,\n\nthanks for reaching out!\n\nbest</email>${securityInstruction}`;
            } else {
...
CRITICAL RULES - VIOLATING ANY OF THESE IS UNACCEPTABLE:
1. DEEP THINKING: First, deeply consider ${fromName}'s identity and intent. 
   - Is this an automated system, platform notification, newsletter, or no-reply address? 
   - If it is a bot or marketing blast, recognize that an informal conversational reply is naive and inappropriate.
   - Evaluate what ${userName} (given their role) would actually want to say here.
   - Put your entire reasoning process inside <think> and </think> tags.
2. OUTPUT ONLY THE FINAL EMAIL TEXT inside <email> and </email> tags. No meta-commentary outside of <think> tags.
3. ADAPT STYLE TO RECIPIENT: If the recipient is a bot/system, use a more functional or professional tone. Do not be conversational with machines. No emojis to bots.
4. Include a professional greeting and sign-off.
5. Keep the email well-formatted (paragraphs, proper capitalization).
6. Address the overall context of the email directly and professionally.
