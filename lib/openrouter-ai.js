...
export class OpenRouterAIService {
    constructor() {
        // Standardized strictly on openrouter/free to guarantee no 404 errors across all key types
            ...
            for (let keyAttempt = 0; keyAttempt < keys.length; keyAttempt++) {
                ...
                try {
                    ...
                    const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
                        ...
                        signal: AbortSignal.timeout(options.timeout || (modelId.includes('free') ? 25000 : 30000))
                    });
    ...
    async generateInboxIntelligence(gmailData, privacyMode = false) {
        ...
        try {
            ...
            return await Promise.race([
                (async () => {
                    const batchSize = 35; // Process up to 35 emails in a single request to avoid concurrency queuing
                    const batches = [];
                    for (let i = 0; i < totalEmails.length; i += batchSize) {
                        batches.push(totalEmails.slice(i, i + batchSize));
                    }

                    console.log(`📡 [Sift AI] Analysis started for ${totalEmails.length} emails in a single batch...`);
                    
                    const batchPromises = batches.map(async (batch, i) => {
                        console.log(`📦 [Sift AI] Processing batch...`);
...
- You MUST classify EVERY SINGLE email provided in the list. Do NOT skip any email under any circumstances.
- If an email does not perfectly fit a specific category, assign it to the closest matching category (e.g., use 'newsletters' for newsletters, mass marketing, automated updates, or social alerts, 'follow_up' for general/ongoing conversations, or 'opportunity' for prospective deals).
- For each and every email in the list, you must output exactly one formatted line.
...
- newsletters: Newsletters, automated digests, product updates, social notifications, or mass promotional emails.
...
STRICT RULE: You must output exactly one line for each and every email ID provided. Do not skip any IDs.`;
