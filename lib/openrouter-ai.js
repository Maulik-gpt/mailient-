    ...
    async generateInboxIntelligence(gmailData, privacyMode = false) {
        ...
        try {
            ...
            return await Promise.race([
                (async () => {
                    const batchSize = 10; // Process 10 emails per batch for better reliability on free models
                    const batches = [];
                    ...
                    for (let i = 0; i < batches.length; i++) {
                        ...
                        const result = await this._processIntelligenceBatch(batches[i], batches[i].map(e => e.id), categories, privacyMode);
