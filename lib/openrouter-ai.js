...
export class OpenRouterAIService {
    ...
    async generateInboxIntelligence(gmailData, privacyMode = false) {
        ...
        // SPEED OPTIMIZATION: Process only 20 most recent emails for <10s response
        const totalEmails = gmailData.slice(0, 20);
        if (totalEmails.length === 0) return this.smartDistribute([], [], categories);

        const startTime = Date.now();

        try {
            // GLOBAL TIMEOUT: The entire process must finish in < 10s
            const GLOBAL_TIMEOUT = 9500;

            return await Promise.race([
                (async () => {
                    const batchSize = 5; // Process 5 emails per batch for individual analysis
                    const batchPromises = [];

                    for (let i = 0; i < totalEmails.length; i += batchSize) {
                        const batch = totalEmails.slice(i, i + batchSize);
                        const batchIds = batch.map(e => e.id);
                        // Process batches sequentially to avoid API overload
                        if (batchPromises.length === 0) {
                            batchPromises.push(this._processIntelligenceBatch(batch, batchIds, categories, privacyMode));
                        } else {
                            // Add small delay between batches
                            const lastPromise = batchPromises[batchPromises.length - 1];
                            batchPromises.push(
                                lastPromise.then(() => 
                    ...
                    console.log(`📡 [Sift AI] Fast analysis started for ${totalEmails.length} emails...`);
                        ...
                        categories.forEach(cat => {
                            ...
                            if (res && (res.summary || (res.actualIds && res.actualIds.length > 0))) {
                                if (isPlaceholder || mergedResult[cat].relevance < (res.relevance || 5)) {
                                    ...
                                    mergedResult[cat].relevance = res.relevance || mergedResult[cat].relevance;
                                }

                                if (Array.isArray(res.actualIds)) {
                                    res.actualIds.forEach(id => {
                                        if (id) mergedResult[cat].ids.push(id);
                                    });
                                }
                            }
                        });
                    ...
                    // Remove local rules fallback - let AI categorize properly

                    // Phase 3: Global Executive Narrative (Optional & Fast)
            ...
            return this.createFallbackResponse(categories, totalEmails, totalEmails.map(e => e.id));
        }
    }
    ...
    async _processIntelligenceBatch(emails, emailIds, categories, privacyMode) {
        ...
        for (let i = 0; i < emails.length; i++) {
            ...
            const content = (email.body || email.snippet || '').substring(0, 200);
...
Categories:
- opportunity: Business opportunities, partnerships, deals
- urgent: Time-sensitive, deadline, emergency
- lead: New customer, inquiry, demo request
- risk: Complaint, issue, cancellation
- follow_up: Needs response, checking in
- important: Confirmations, receipts, alerts
...
Return only the category name:`;
            ...
            try {
                ...
                const category = this.extractResponse(response).toLowerCase().trim();
                console.log(`📧 [Sift AI] Email ${i + 1}: ${category}`);
                
                // Map response to valid category
                let validCategory = null;
                if (category.includes('opportunity')) validCategory = 'opportunity';
                else if (category.includes('urgent')) validCategory = 'urgent';
                else if (category.includes('lead')) validCategory = 'lead';
                else if (category.includes('risk')) validCategory = 'risk';
                else if (category.includes('follow')) validCategory = 'follow_up';
                else if (category.includes('important')) validCategory = 'important';
                
                if (validCategory && result[validCategory]) {
                    result[validCategory].ids.push((i + 1).toString());
                    result[validCategory].actualIds.push(emailId);
                    result[validCategory].relevance = Math.max(5, result[validCategory].relevance);
                }
            } catch (e) {
                console.error(`❌ [Sift AI] Email ${i + 1} failed: ${e.message}`);
                // Continue with next email
            }
        }

        // Generate summaries for categories that have emails
        categories.forEach(cat => {
            if (result[cat].ids.length > 0) {
                result[cat].summary = `${cat.charAt(0).toUpperCase() + cat.slice(1)} (${result[cat].ids.length})`;
                result[cat].description = `${result[cat].ids.length} emails categorized as ${cat}`;
                result[cat].relevance = Math.min(10, 5 + result[cat].ids.length);
            }
        });

        console.log(`📊 [Sift AI] Final categorization:`, Object.entries(result).map(([cat, data]) => `${cat}: ${data.ids.length} emails`));
        return result;
    }
