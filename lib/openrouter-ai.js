...
TONE: Professional, insightful, and extremely concise. Keep the total response under 150 words.`
    ...
    async enhanceNote(subject, content) {
        try {
            ...
            if (!apiKey) {
                console.error('❌ No OPENROUTER_API_KEY configured for note enhancement');
...
You MUST return a JSON object with exactly two keys:
{
  ...
  "content": "The architecturally structured markdown content"
            ...
            // Not using "json_object" format as it's not supported by all models or can be too strict
            ...
            if (!result) return { subject, content };
            ...
            try {
                ...
                // If it parsed but missing fields, try to salvage
                if (parsed.content) {
                    return {
                ...
                console.error('Failed to parse enhanced note JSON. Raw result:', result);
                // Fallback: if result contains a subject-like line at the start, try to split it
                return { subject, content };
            }
        } catch (error) {
