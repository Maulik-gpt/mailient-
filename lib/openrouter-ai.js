...
4. **NAME INCORPORATION**: ALWAYS start with a greeting that includes ${fromName}'s name. ALWAYS end with a sign-off followed by your name (${userName}).
5. **VOICE ADHERENCE**: If the user profile is casual, be casual but *intelligent*. Match the profile perfectly.
6. **SIGNOFFS**: Use the sign-offs listed in the voice profile, but don't forget to add your name (${userName}) after it.
...
<email>[Substantive, high-quality draft reply]</email>${securityInstruction}`;
            } else {
...
3. **OUTPUT FORMAT**: Output ONLY the final email text inside <email> and </email> tags. No meta-commentary.
4. **GREETING & SIGN-OFF**: Start with a professional greeting including ${fromName}'s name. End with a professional sign-off and your name: ${userName}.
...
OUTPUT FORMAT (Must be exactly this structure):
<think>[Rigorous analysis of sender, intent, and strategic response path]</think>
<email>[Substantive, professional draft reply]</email>`;
            }
