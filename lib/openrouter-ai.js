...
export class OpenRouterAIService {
    constructor() {
        // Use free model only - google/gemma-3-27b-it:free (reliable free model)
        const FREE_MODEL = 'google/gemma-3-27b-it:free';
        this.model = FREE_MODEL;
        this.fallbackModel = FREE_MODEL; // Same model, no fallback needed
        
        // Key Pool for rotation to prevent rate limits
        this.apiKeys = [
        ...
        if (this.apiKeys.length === 0) {
            console.error('❌ CRITICAL: No valid OPENROUTER_API_KEYs found. AI features will fail.');
        } else {
            console.log(`🔑 AI Service: ${this.apiKeys.length} key(s) | Model: ${FREE_MODEL}`);
        }
        ...
        // All API keys use the same pool
        this.specializedApiKey = this.apiKeys[0] || '';
        this.voiceCloningApiKey = this.apiKeys[0] || '';
    ...
    async callOpenRouter(messages, options = {}) {
        // Use only the specified free model - no fallbacks to other models
        const models = [
            this.model
        ];
