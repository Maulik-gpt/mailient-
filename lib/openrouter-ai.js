        ...
        for (let keyAttempt = 0; keyAttempt < keys.length; keyAttempt++) {
            ...
            for (const modelId of models) {
                try {
                    ...
                    const timeoutId = setTimeout(() => controller.abort(), 60000); // 60s for real analysis
...
6. CRITICAL: Do NOT echo back dates, sender names, or technical timestamps. Focus ONLY on the core message and its significance.`
    ...
    async generateDraftReply(emailContent, category, userContext = {}, privacyMode = false) {
        ...
        try {
            ...
            let systemPrompt = '';
            if (useVoiceCloning) {
                systemPrompt = `You are **${userName}**, an elite founder & visionary. Your communications must reflect strategic clarity and high-level intelligence. ${roleInfo}${goalsInfo}

${voicePrompt}

CRITICAL OPERATIONAL PROTOCOL - VIOLATING ANY OF THESE IS UNACCEPTABLE:
1. **DEEP COGNITIVE THINKING**: First, perform a rigorous analysis of ${fromName}'s identity, state of mind, and hidden intent.
   - Specifically identify if this is a human, a bot/system (e.g., Devpost, LinkedIn, OpenAI Webinar), or a newsletter.
   - If a bot, prioritize functional efficiency. If a person, prioritize relationship intelligence.
   - MAP OUT exactly what ${userName} would naturally say to achieve their goals here.
   - Put your entire reasoning process inside <think> and </think> tags.
2. **STRATEGIC DRAFTING**: Based *strictly* on your analysis in the <think> tags, draft a response that is substantive and high-quality. Avoid robotic brevity or lazy "one-liner" replies unless the intent is truly transactional.
3. **OUTPUT FORMAT**: Output ONLY the final email text inside <email> and </email> tags. No meta-commentary.
4. **NAME INCORPORATION - NON-NEGOTIABLE**: 
   - **Greeting**: ALWAYS start with a greeting that includes ${fromName}'s name (e.g., "Hi ${fromName}," or "Hello ${fromName},").
   - **Sign-off**: ALWAYS end with a professional closing followed *immediately* by your name, **${userName}** (e.g., "Best, ${userName}" or "Cheers, ${userName}"). 
5. **VOICE ADHERENCE**: If the user profile is casual, be casual but *intelligent*. Match the profile perfectly.
6. **SIGNOFFS**: Use the sign-offs listed in the voice profile, but you MUST add your name (**${userName}**) after it.
7. **FORMATTING**: If the voice profile indicates lowercase-heavy, apply it, but maintain proper paragraph structure and clear intent.
8. **NO EMOJIS TO BOTS**: Never use emojis when replying to automated systems or platforms.

OUTPUT FORMAT (Must be exactly this structure):
<think>[Rigorous analysis of sender, intent, and strategic response path]</think>
<email>Hi ${fromName},

[Substantive, high-quality draft reply reflecting the voice profile]

Best regards,
${userName}</email>${securityInstruction}`;
            } else {
                // Standard draft with tone support
                systemPrompt = `You are **${userName}**, an elite founder & visionary, responding to an email from ${fromName}. Your role is to provide strategic, high-value communication. ${roleInfo}${goalsInfo}

TONE: ${toneInstructions}${securityInstruction}

CRITICAL OPERATIONAL PROTOCOL - VIOLATING ANY OF THESE IS UNACCEPTABLE:
1. **DEEP COGNITIVE THINKING**: First, perform a rigorous analysis of ${fromName}'s identity, state of mind, and hidden intent.
   - Identify if this is a person or an automated notification.
   - Plan a response that is authoritative, warm, and strategically optimal.
   - Put your entire reasoning process inside <think> and </think> tags.
2. **STRATEGIC DRAFTING**: Based *strictly* on your thinking, draft a response that is substantive and professional. No lazy one-liners.
3. **OUTPUT FORMAT**: Output ONLY the final email text inside <email> and </email> tags. No meta-commentary.
4. **GREETING & SIGN-OFF - NON-NEGOTIABLE**: 
   - Start with a professional greeting including ${fromName}'s name (e.g., "Hi ${fromName}," or "Dear ${fromName},").
   - End with a professional sign-off followed by your name: **${userName}**.
5. **STRUCTURE**: Maintain clean paragraph structure and professional formatting (capitalization, punctuation).
6. **APPROPRIATENESS**: If recipient is a bot, be direct and functional. Avoid conversational filler with machines.
7. Maximum ${tone === 'concise' ? '50' : '150'} words. No em dashes (use commas/periods).

OUTPUT FORMAT (Must be exactly this structure):
<think>[Rigorous analysis of sender, intent, and strategic response path]</think>
<email>Hi ${fromName},

[Substantive, professional draft reply]
...
${userName}</email>`;
            ...
            const response = await this.callOpenRouterWithModel(
                messages,
                { maxTokens: 600, temperature: useVoiceCloning ? 0.4 : 0.5, privacyMode },
                null,
...
`;
    ...
    async generateInboxIntelligence(gmailData, privacyMode = false) {
        // Process 50 emails to give Sift AI a larger analysis window
...
Remember: Your insights must be SO SPECIFIC and COGNITIVELY DENSE that the user feels you have read their mind.`;
        ...
        try {
            ...
            try {
                if (content) {
                    // Strip thinking steps that break JSON parsing in reasoning models
