        ...
        try {
            ...
            batchResults.forEach(batchResult => {
                ...
                categories.forEach(cat => {
                    ...
                    if (res?.summary && res.summary !== mergedResult[cat].summary) {
                        // Priority: Keep higher relevance or non-default summaries
                        if (mergedResult[cat].relevance < (res.relevance || 0) || mergedResult[cat].summary.includes('Scanning')) {
                            mergedResult[cat].summary = res.summary;
                            mergedResult[cat].description = res.description || res.summary;
                            mergedResult[cat].relevance = Math.max(mergedResult[cat].relevance, res.relevance || 5);
...
${emailList}`;
        ...
        try {
            const response = await this.callOpenRouter([{ role: 'user', content: prompt }], {
                ...
                maxTokens: 1000,
                privacyMode,
                model: 'google/gemma-2-9b-it:free', // Using 9B for parallel batches: much faster & stable
                timeout: 9000, 
                singleModel: true 
            });
            ...
            console.warn(`⚠️ [Sift AI] Batch failed: ${e.message}`);
            return null;
        }
    }
    ...
    async _generateGlobalNarrative(mergedResult, categories, privacyMode) {
        ...
        try {
            const response = await this.callOpenRouter([
                ...
                privacyMode,
                model: 'google/gemma-4-31b-it:free',
                timeout: 3000, // Very fast synthesis
                singleModel: true
            });
    ...
    createFallbackResponse(categories, emails = [], emailIds = []) {
        ...
        categories.forEach(category => {
            const description = categoryDescriptions[category] || 'Scanning your inbox...';
            fallback[category] = { 
                title: category, 
                summary: description, 
