...
7. CRITICAL: Deliver exactly 3-4 sentences of deep insight. No more, no less.`
            ...
            const response = await this.callOpenRouterWithModel(messages, { 
                ...
                temperature: 0.3, 
                privacyMode, 
                model: 'openrouter/free', // Use auto-routed free model for fastest response
                ...options 
            });
    ...
    async generateInboxIntelligence(gmailData, privacyMode = false) {
        ...
        try {
            ...
            const response = await this.callOpenRouter(messages, { 
                ...
                timeout: 10000, // 10s limit for fast Sift AI
            ...
            if (!enhancedContent || enhancedContent === 'Identified items' || enhancedContent === 'Reviewing inbox for relevant items...') {
                if (ids.length > 0) {
                    const categoryPhrases = {
                        opportunity: `Discovered metadata in ${ids.length} communications indicating strategic growth, potential revenue events, or high-value partnerships.`,
                        urgent: `Identified ${ids.length} mission-critical items with high-urgency signals such as pending deadlines or blocking issues.`,
                        lead: `Captured ${ids.length} new business prospects or sales leads requiring immediate nurturing.`,
                        risk: `Detected ${ids.length} potential operational risks, including client concerns or budget flags.`,
                        follow_up: `Noted ${ids.length} conversations requiring coordination to maintain professional momentum.`,
                        important: `Organized ${ids.length} key informational updates or financial confirmations.`
                    };
                    enhancedContent = categoryPhrases[category] || `${ids.length} specific items identified via contextual analysis.`;
                } else {
