                                        ...
                                        if (!isInsideThink) {
                                            if (isInsideEmail) {
                                                ...
                                                if (buffer.length > 15 || buffer.includes('\n')) {
                                                    ...
                                                    if (/^(Subject|Re|To|From|Sent|Date)\s*:\s*/i.test(cleanLine)) {
                                                        buffer = ""; // Skip this line
                                                    } else {
                                                        controller.enqueue(encoder.encode(buffer));
                                                        buffer = "";
                                                    }
    ...
    async generateEmailSummary(emailContent, privacyMode = false, userContext = {}, options = {}) {
        ...
        try {
            ...
            const response = await this.callOpenRouterWithModel(messages, { 
                ...
                model: options.model || 'google/gemma-2-9b-it:free', // Fastest free model
                ...options 
            });
    ...
    async generateDraftReply(emailContent, category, userContext = {}, privacyMode = false, options = {}) {
        ...
        try {
            ...
            const response = await this.callOpenRouterWithModel(messages, { 
                ...
                privacyMode, 
                timeout: 15000, // 15s cap for draft generation
                singleModel: true,
...
EMAILS:\n${emailList}`;
    ...
    getDraftReplyPrompt(userName, fromName, roleInfo, goalsInfo, toneInstructions, securityInstruction, voicePrompt, useVoiceCloning, tone) {
        return `Draft reply for ${userName} (${roleInfo}). ${voicePrompt}\n${toneInstructions}\nRULES: Think in <think>, reply in <email>. Greet ${fromName}. Sign ${userName}. NO Subject/Re lines.${securityInstruction}`;
    }
