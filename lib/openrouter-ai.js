...
export class OpenRouterAIService {
    ...
    async generateInboxIntelligence(gmailData, privacyMode = false) {
        ...
        // Phase 1: Parallel Batching - handle 40 emails in <10s to avoid 504 timeouts
...
${emailList}`;
        ...
        try {
            const response = await this.callOpenRouter([{ role: 'user', content: prompt }], {
                ...
                privacyMode,
                model: 'google/gemma-4-31b-it:free',
                timeout: 8000, // Strict 8s per batch
                singleModel: true // Don't waste time on fallbacks within a batch
            });
