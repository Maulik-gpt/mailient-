...
After the </think> tag, output ONLY the final 2-3 sentence summary. No JSON, no labels, no formatting in the final output.`
    ...
    async generateInboxIntelligence(gmailData, privacyMode = false) {
        ...
        try {
            ...
            // Process batches sequentially to avoid OpenRouter free tier rate limits
            const batchResults = [];
            for (let i = 0; i < batches.length; i++) {
                ...
                let result = null;
                let retries = 0;
                const MAX_RETRIES = 2; // Actually retry to survive transient errors
...
OUTPUT FORMAT:
You MUST respond with a STRICT JSON array of objects. Do not include any conversational filler, markdown formatting, or thoughts. Just the JSON array.
[
  {
    "id": "<email_id>",
    "category": "<category>",
