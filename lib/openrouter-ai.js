...
export class OpenRouterAIService {
    constructor() {
        // Use free model - nvidia/nemotron-3-super-120b-a12b:free (High-fidelity nemesis model)
        const FREE_MODEL = "nvidia/nemotron-3-super-120b-a12b:free"; // High-fidelity nemesis model
        this.model = FREE_MODEL;
        this.fallbackModel = FREE_MODEL;
        ...
        // All features use the same free model
        this.specializedModel = FREE_MODEL;
        this.voiceCloningModel = FREE_MODEL;
    ...
    async callOpenRouter(messages, options = {}) {
        // Robust model fallback chain for free models
        const models = [
            process.env.OPENROUTER_MODEL,         // User preferred model
            this.model,                           // Default model (nvidia/nemotron-3-super-120b-a12b:free)
            'meta-llama/llama-3.1-8b-instruct:free', // Classic fallback
            'google/gemma-2-9b-it:free',          // Highly reliable fallback
            'nvidia/nemotron-3-nano-30b-a3b:free', // Fast fallback
            'mistralai/mistral-7b-instruct:free',  // Classic fallback
            'microsoft/phi-3-mini-128k-instruct:free' // Tiny fallback
        ].filter(Boolean);
        ...
        for (let keyAttempt = 0; keyAttempt < keys.length; keyAttempt++) {
            ...
            for (const modelId of models) {
                try {
                    ...
                    const timeoutId = setTimeout(() => controller.abort(), 20000); // 20s timeout for free models
...
- Sign-off with ${userName}'s name`;
            ...
            const response = await this.callOpenRouterWithModel(
                messages, 
                { maxTokens: 800, temperature: useVoiceCloning ? 0.4 : 0.5, privacyMode }, 
                model, 
...
Remember: Your insights must be SO SPECIFIC and COGNITIVELY DENSE that the user feels you have read their mind.`;
    ...
    buildEnhancedResponse(result, emailIds, categories) {
        const insights = categories.map(category => {
            ...
            if (!enhancedContent || enhancedContent === 'Identified items') {
                if (ids.length > 0) {
                    const categoryPhrases = {
                        opportunity: `Discovered ${ids.length} potential growth or revenue opportunities in your recent communications.`,
                        urgent: `Identified ${ids.length} time-sensitive items requiring your immediate attention.`,
                        lead: `Captured ${ids.length} new leads or business prospects that should be nurtured.`,
                        risk: `Detected ${ids.length} potential business risks or relationship concerns to address.`,
                        follow_up: `Noted ${ids.length} conversations where a follow-up would help maintain momentum.`,
                        important: `Organized ${ids.length} key informational updates and high-level communications.`
                    };
                    enhancedContent = categoryPhrases[category] || `${ids.length} items identified needing review.`;
                } else {
                    enhancedContent = 'No significant items identified in this category from your recent emails.';
                }
            }
...
** RULES **:
    ...
    - Focus on moving forward, not rehashing issues`;
            ...
            const response = await this.callOpenRouterWithModel(
                messages, 
                { maxTokens: 600, temperature: useVoiceCloning ? 0.4 : 0.7, privacyMode }, 
                model, 
...
** RULES **:
    ...
    - Reference actual content from the original email`;
            ...
            const response = await this.callOpenRouterWithModel(
                messages, 
                { maxTokens: 500, temperature: useVoiceCloning ? 0.4 : 0.6, privacyMode }, 
                model, 
                null // Use rotation pool
            );
...
TONE: Professional, insightful, and extremely concise. Keep the total response under 150 words.`
            ...
            const response = await this.callOpenRouterWithModel(messages, { maxTokens: 400, temperature: 0.4, privacyMode }, this.specializedModel, null);
