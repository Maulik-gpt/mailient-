...
export class OpenRouterAIService {
    ...
    async callOpenRouter(messages, options = {}) {
        ...
        // Enforce a strict limit of 2 total network attempts to keep response times under 10s
        let totalAttempts = 0;
    ...
    async generateEmailSummary(emailContent, privacyMode = false, userContext = {}, options = {}) {
        ...
        try {
            ...
            const response = await this.callOpenRouterWithModel(messages, {
                ...
                temperature: 0.1,
                privacyMode,
                timeout: 7000,  // 7s hard cap — summary must be snappy
                singleModel: false, // Allow fallback to 26b if 31b fails/times out
    ...
    async generateDraftReply(emailContent, category, userContext = {}, privacyMode = false, options = {}) {
        ...
        try {
            ...
            const response = await this.callOpenRouterWithModel(messages, {
                ...
                privacyMode,
                timeout: 18000, // 18s cap — slightly longer for draft reliability
                singleModel: true, // Don't cascade through multiple models
    ...
    async generateInboxIntelligence(gmailData, privacyMode = false) {
        ...
        // Phase 1: Balanced Batching - 2 batches of 20 emails
        // Optimized for stability on Free-tier models
        const totalEmails = gmailData.slice(0, 40);
        if (totalEmails.length === 0) return this.smartDistribute([], [], categories);

        const batchSize = 20;
        const batchPromises = [];
        
        for (let i = 0; i < totalEmails.length; i += batchSize) {
            const batch = totalEmails.slice(i, i + batchSize);
            const batchIds = batch.map(e => e.id);
            // Stagger requests slightly (300ms) to avoid triggering burst rate limits
            const delay = i === 0 ? 0 : 300;
            batchPromises.push(new Promise(resolve => setTimeout(resolve, delay)).then(() => 
                this._processIntelligenceBatch(batch, batchIds, categories, privacyMode)
            ));
        }

        console.log(`📡 [Sift AI] Deep analysis started for ${totalEmails.length} emails using ${batchPromises.length} parallel batches...`);
        const startTime = Date.now();

        try {
            // Safety: Race all batches with a strict 8s timeout to prevent 504 errors
            const batchResults = await Promise.all(batchPromises);
            const duration = (Date.now() - startTime) / 1000;
            console.log(`✅ [Sift AI] Analysis complete in ${duration.toFixed(2)}s`);

            // Phase 2: Intelligence Merging & Deep Synthesis
            const mergedResult = this.createFallbackResponse(categories, totalEmails, totalEmails.map(e => e.id));
            
            batchResults.forEach(batchResult => {
                if (!batchResult) return;
                categories.forEach(cat => {
                    const res = batchResult[cat];
                    // Logic Fix: Any valid AI summary or IDs should overwrite the placeholder
                    const isPlaceholder = mergedResult[cat].summary === 'Scanning...' || mergedResult[cat].summary.includes('found in your inbox');
                    
                    if (res && (res.summary || (res.actualIds && res.actualIds.length > 0))) {
                        // Priority: AI results always overwrite placeholders or lower-relevance local rules
                        if (isPlaceholder || mergedResult[cat].relevance < (res.relevance || 5)) {
                            ...
                            mergedResult[cat].relevance = Math.max(mergedResult[cat].relevance, res.relevance || 7);
                            mergedResult[cat].action_item = res.action_item;
                            mergedResult[cat].sentiment = res.sentiment;
                        }
                        
                        // Accumulate all IDs from all batches
                        if (Array.isArray(res.actualIds)) {
                            res.actualIds.forEach(id => {
                                if (!mergedResult[cat].ids.includes(id)) mergedResult[cat].ids.push(id);
                            });
                        }
                    }
                });
            ...
            // Refinement: If a category has many IDs but a generic fallback summary,
            // try to make it slightly more descriptive from the local context
            categories.forEach(cat => {
                const res = mergedResult[cat];
                if (res.ids.length > 0 && res.summary.includes('Scanning')) {
                    const sampleEmails = totalEmails.filter(e => res.ids.includes(e.id)).slice(0, 2);
                    if (sampleEmails.length > 0) {
                        res.summary = `Items regarding ${sampleEmails.map(e => e.subject.substring(0, 30)).join(', ')}`;
                        res.description = `Analyzed ${res.ids.length} emails including threads from ${sampleEmails.map(e => e.from.split('<')[0]).join(' and ')}.`;
                    }
                }
            });
            
            // Phase 3: Global Executive Narrative
            ...
            if (narrative) response.sift_narrative = narrative;
            
            return response;
        } catch (error) {
            console.error('🚀 [Sift AI] Critical failure or timeout in SIFT pipeline:', error.message);
            const emailIds = totalEmails.map(e => e.id);
            return this.smartDistribute(totalEmails, emailIds, categories);
        }
    }

    /**
     * Internal helper to process a small batch of emails
     */
    async _processIntelligenceBatch(emails, emailIds, categories, privacyMode) {
        const emailList = emails.map((e, idx) =>
            `ID: ${idx + 1} | From: ${e.from} | Subject: ${e.subject} | Content: ${(e.snippet || '').substring(0, 150)}\n`
        ).join('\n---\n');

        const prompt = `Analyze these ${emails.length} emails with deep business intelligence. 
Categorize into: ${categories.join(', ')}.
For each match, identify:
1. summary: One-line title.
2. description: 2-3 sentence logical thread.
3. action_item: The single most important next step.
4. sentiment: positive, negative, or neutral.
5. relevance: 1-10.
6. ids: ["ID_NUMBER"]

Output ONLY JSON: {"category": {"summary": "...", "description": "...", "action_item": "...", "sentiment": "...", "relevance": 10, "ids": ["1"]}}
...
${emailList}`;
        ...
        try {
            const response = await this.callOpenRouter([{ role: 'user', content: prompt }], {
                ...
                maxTokens: 1000,
                privacyMode,
                model: 'tencent/hy3-preview:free', // High-performance Tencent model as requested
                timeout: 10000, 
                singleModel: true 
            });
            ...
            // Map relative IDs back to actual Gmail IDs immediately
            Object.keys(aiResult).forEach(cat => {
                const res = aiResult[cat];
                if (res && Array.isArray(res.ids)) {
                    res.actualIds = res.ids
                        .map(id => emailIds[parseInt(id) - 1])
                        .filter(Boolean);
                }
            });
            ...
            console.error(`❌ [Sift AI] Batch processing failed critical error: ${e.message}`);
            return null;
        }
    }
