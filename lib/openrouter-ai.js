    ...
    async generateEmailSummary(emailContent, privacyMode = false, userContext = {}, options = {}) {
        ...
        try {
            ...
            const messages = [
                {
                    role: 'system',
                    content: `SYSTEM: Executive Analyst.\nCONTEXT: ${roleContext}${goalContext}\nOBJECTIVE: Write a clear, insightful 3-5 sentence summary of this email as plain prose. Focus on what matters: who wrote it, what they want, and any action needed. Output ONLY the summary text — no JSON, no labels like "Summary:", no code blocks, no markdown formatting. Just clean readable sentences.`
                },
                { role: 'user', content: `Summarize this email:\n\n${cleanContent}` }
            ];

            const response = await this.callOpenRouterWithModel(messages, { maxTokens: 300, temperature: 0.2, privacyMode, ...options });
