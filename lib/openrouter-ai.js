...
export class OpenRouterAIService {
    constructor() {
        // Use google/gemini-2.0-flash:free for premium structured intelligence and speed
        this.model = 'google/gemini-2.0-flash:free';
        this.fallbackModels = ['google/gemini-2.0-flash:free', 'google/gemini-2.0-flash-lite:free', 'openrouter/free'];
        ...
        this.specializedModel = 'google/gemini-2.0-flash:free';
        this.voiceCloningModel = 'google/gemini-2.0-flash:free';
        ...
        try {
            ...
            return await Promise.race([
                (async () => {
                    ...
                    // Phase 2.6: Build final descriptions from ALL reasons (AI + Fallback)
                    categories.forEach(cat => {
                        if (mergedResult[cat].reasons && mergedResult[cat].reasons.length > 0) {
                            // Take top 4 most unique reasons
                            const uniqueReasons = [...new Set(mergedResult[cat].reasons)].slice(0, 4);
                            mergedResult[cat].description = uniqueReasons.map(r => `• ${r}`).join("\n");
                        } else if (mergedResult[cat].ids.length > 0) {
                            mergedResult[cat].description = `• Significant activity detected in ${cat}. Review for details.`;
                        }
                    });
            ...
            // Phase 2.6: Build final descriptions for fallback
            categories.forEach(cat => {
                if (fallbackResult[cat].reasons && fallbackResult[cat].reasons.length > 0) {
                    const uniqueReasons = [...new Set(fallbackResult[cat].reasons)].slice(0, 4);
                    fallbackResult[cat].description = uniqueReasons.map(r => `• ${r}`).join("\n");
                } else if (fallbackResult[cat].ids.length > 0) {
                    fallbackResult[cat].description = `• ${cat.charAt(0).toUpperCase() + cat.slice(1)} detected via local filters.`;
                }
            });
