...
export class OpenRouterAIService {
            ...
            for (const modelId of models) {
                ...
                try {
                    ...
                    const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
                        ...
                        headers: {
                            "Authorization": `Bearer ${apiKey}`,
                            "HTTP-Referer": "https://mailient.com",
                            "X-Title": "Mailient AI",
                            "Content-Type": "application/json"
                        },
                        body: JSON.stringify({
    ...
    async generateInboxIntelligence(gmailData, privacyMode = false) {
        // Optimized: 20 emails (down from 30) to guarantee JSON reliability and sub-10s analysis
        const emails = gmailData.slice(0, 20);
        ...
        const prompt = `Advanced Business Email Analyst. Your task is to analyze exactly ${emails.length} emails.
You MUST provide a detailed categorization for EVERY email that fits. DO NOT skip emails.
For each category found, you MUST provide a "description" field that explains the logical thread across those specific emails.
...
${emailList}`;
        ...
        try {
            const response = await this.callOpenRouter([{ role: 'user', content: prompt }], { 
                ...
                privacyMode,
                model: 'google/gemma-4-31b-it:free',
                timeout: 40000 // 40s total budget for complex analysis
            });
