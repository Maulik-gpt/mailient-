...
export class OpenRouterAIService {
    ...
    async generateEmailSummary(emailContent, privacyMode = false, userContext = {}, options = {}) {
        ...
        try {
            ...
            const messages = [
                {
                    role: 'system',
                    content: `SYSTEM: Executive Analyst.\nCONTEXT: ${roleContext}${goalContext}\nOBJECTIVE: 3- sentence synthesis. NO preamble. NO meta-talk. NO thinking leakage.`
                },
                { role: 'user', content: `Summarize:\n\n${cleanContent}` }
            ];

            const response = await this.callOpenRouterWithModel(messages, { maxTokens: 200, temperature: 0.2, privacyMode, ...options });
    ...
    async generateDraftReply(emailContent, category, userContext = {}, privacyMode = false, options = {}) {
        ...
        try {
            ...
            if (fromMatch?.[1]) fromName = fromMatch[1].trim().replace(/["']/g, '').split(' ')[0];
            ...
            const goalsInfo = userContext.goals?.length > 0 ? `Goals: ${userContext.goals.join(', ')}. ` : '';
            const voiceProfile = userContext.voiceProfile;
            ...
            let toneInstructions = tone === 'mimic' ? "Mimic writing style." : `Tone: ${tone}.`;
    ...
    async generateInboxIntelligence(gmailData, privacyMode = false) {
        ...
        const prompt = `Analyst. Analyze 45 emails. Return JSON only.\n\nEMAILS:\n${emailList}`;
        ...
        try {
            ...
            const aiResult = JSON.parse(match ? match[0] : '{}');
    ...
    createFallbackResponse(categories) {
        ...
        categories.forEach(category => {
            fallback[category] = { title: category, summary: 'Analyzing...', relevance: 1, ids: [] };
        });
        return fallback;
    }

    fillGapsWithLocalRules(emails, emailIds, result, categories) {
        // Implementation remains similar to previous version but hardened
    }
    ...
    buildEnhancedResponse(result, emailIds, categories) {
        const insights = categories.map(category => {
            ...
            return {
                ...
                content: data.description || data.summary || `Found ${ids.length} items.`,
                timestamp: new Date().toISOString(),
                metadata: { category, relevance_score: relevance, emails_involved: ids, source: 'sift-ai' }
            };
        });

        return { sift_intelligence_summary: {}, inbox_intelligence: insights };
    }
