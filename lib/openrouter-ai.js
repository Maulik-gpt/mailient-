...
ID: <id> | CATEGORY: <category> | REASON: <action-oriented reason>
...
STRICT RULE: Only output the formatted lines for relevant emails. Silence for everything else.`;
        ...
        try {
            ...
            const results = lines.map(line => {
                ...
                let reason = '';
                const reasonMatch = line.match(/(?:REASON|REACTION|SUMMARY|DESC|DESCRIPTION):\s*(.*)$/i);
                if (reasonMatch) {
                    reason = reasonMatch[1].trim();
                } else {
                    // Robust fallback: if no explicit keyword but we have at least 3 pipe-separated parts, use the 3rd part
