...
export class OpenRouterAIService {
    constructor() {
        ...
        // SIFT AI Model (Intelligence/Prioritization) - Kept original as requested
        this.model = 'arcee-ai/trinity-large-preview:free';

        // Specialized Tasks Model (Summary, Drafts, Notes, etc.) - Switched to fast StepFun
        this.specializedModel = 'stepfun/step-3.5-flash:free';
        this.specializedApiKey = (process.env.OPENROUTER_API_KEY || process.env.OPENROUTER_API_KEY2 || process.env.OPENROUTER_API_KEY3 || '').trim();
        this.baseURL = 'https://openrouter.ai/api/v1';

        // Voice Cloning uses the specialized model for consistency
        this.voiceCloningApiKey = (process.env.OPENROUTER_API_KEY || process.env.OPENROUTER_API_KEY2 || process.env.OPENROUTER_API_KEY3 || '').trim();
        this.voiceCloningModel = 'stepfun/step-3.5-flash:free';
        ...
        if (this.model && !this.model.includes('/')) {
            console.warn(`⚠️ Invalid model format "${this.model}". Falling back to StepFun Step-3.5-Flash.`);
    ...
    async callOpenRouter(messages, options = {}) {
        ...
        // Fallback: meta-llama/llama-3.1-8b-instruct:free — widely available free model
        const models = [
            this.model || 'stepfun/step-3.5-flash:free',
        ...
        for (const modelId of models) {
            try {
                ...
                // 25 second timeout — free models (StepFun) can take 10-20s
                ...
                if (error.name === 'AbortError') {
                    console.warn('⏱️ ' + modelId + ' timed out after 8s');
                } else {
    ...
    async callOpenRouterWithModel(messages, options = {}, model = null, apiKey = null) {
        // Fallback chain for specialized models
        const models = [
            model || this.specializedModel || 'stepfun/step-3.5-flash:free',
            'meta-llama/llama-3.1-8b-instruct:free'
        ];

        console.log('🤖 AI Request (Specialized) started with ' + messages.length + ' messages');
        ...
        for (const modelId of models) {
            try {
                ...
                // Increased timeout for specialized calls
                const controller = new AbortController();
                const timeoutId = setTimeout(() => controller.abort(), 25000);
