...
TONE: Professional, insightful, and extremely concise. Keep the total response under 150 words.`
    ...
    async enhanceNote(subject, content) {
        try {
            const apiKey = (process.env.OPENROUTER_API_KEY3 || process.env.OPENROUTER_API_KEY || process.env.OPENROUTER_API_KEY2 || '').trim();
            // Use a more stable flash model
            const model = 'google/gemini-2.0-flash-lite-preview-02-05:free';

            if (!apiKey) {
                console.error('❌ AI SERVICE: No API key found for enhanceNote');
            ...
            const messages = [
                {
                    role: 'system',
                    content: `You are an elite Cognitive Architect. Transform the user's raw note into a high-density, professionally structured asset.
                    
Return ONLY a valid JSON object with:
{
  "subject": "A punchy, insightful title",
  "content": "Well-structured markdown body"
}`
                },
                {
                    role: 'user',
            ...
            console.log('📡 AI SERVICE: Calling OpenRouter for note enhancement...');
            ...
            console.log('📡 AI SERVICE: Raw response content length:', result?.length || 0);

            if (!result) {
                console.error('❌ AI SERVICE: Received empty response from model');
                return { subject, content };
            }
            ...
            try {
                ...
                // If there's still extra text outside the braces, find the first '{' and last '}'
                ...
                if (parsed.subject && parsed.content) {
                    console.log('✅ AI SERVICE: Successfully parsed enhanced note');
                    return {
                ...
                console.warn('⚠️ AI SERVICE: Parsed JSON missing required fields');
                return { subject, content };
            } catch (parseError) {
                console.error('❌ AI SERVICE: JSON parse error. Raw result:', result);
                return { subject, content };
            }
        } catch (error) {
            console.error('❌ AI SERVICE: Fatal error in enhanceNote:', error);
            return { subject, content };
        }
    }
}
