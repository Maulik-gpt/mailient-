...
export class OpenRouterAIService {
    ...
    async generateInboxIntelligence(gmailData, privacyMode = false) {
        ...
        // Phase 1: High-Speed Batching - limit to 30 emails for near-instant response
        const totalEmails = gmailData.slice(0, 30);
        ...
        try {
            // Safety: Race all batches with a strict 45s timeout to prevent 504 errors
            const BATCH_TIMEOUT = 45000;
            const batchResults = await Promise.race([
                Promise.all(batchPromises),
                new Promise((_, reject) =>
                    setTimeout(() => reject(new Error('Sift batch analysis timed out')), BATCH_TIMEOUT)
                )
            ]);
    ...
    async _processIntelligenceBatch(emails, emailIds, categories, privacyMode) {
        const emailList = emails.map((e, idx) => {
            const content = (e.body || e.snippet || '').substring(0, 500);
            return `ID: ${idx + 1} | From: ${e.from} | Subject: ${e.subject} | Content: ${content}\n`;
        }).join('\n---\n');

        const prompt = `You are a business intelligence analyst. Carefully analyze these ${emails.length} emails and categorize them.

Categories: ${categories.join(', ')}

For each email that matches a category, provide:
1. summary: A concise, descriptive title (max 15 words)
2. description: Detailed 2-3 sentence analysis explaining the business context and implications
3. action_item: Specific, actionable next step required
4. sentiment: positive, negative, or neutral
5. relevance: Score 1-10 based on business importance
6. ids: Email ID number(s) that match this category

REQUIREMENTS:
- description MUST be detailed and meaningful, not generic
- Only include categories that have actual matching emails
- Ensure each description explains WHY this email belongs to the category

Output ONLY valid JSON:
{"category_name": {"summary": "Descriptive title", "description": "Detailed business analysis explaining the context and why this matters", "action_item": "Specific action required", "sentiment": "positive/negative/neutral", "relevance": 8, "ids": ["1", "2"]}}
...
${emailList}`;
        ...
        try {
            const response = await this.callOpenRouter([{ role: 'user', content: prompt }], {
                ...
                timeout: 25000, // 25s per batch to allow for more detailed analysis
                singleModel: true // Disable fallback to keep speed ultra-high
            });
