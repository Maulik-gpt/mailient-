...
export class OpenRouterAIService {
    ...
    async callOpenRouterStream(messages, options = {}) {
        ...
        return response.body; // This is a ReadableStream in Node/Edge
    }
...
`;
    ...
    async generateInboxIntelligence(gmailData, privacyMode = false) {
        ...
        const runBatch = async (batchEmails, batchIndex, offset) => {
            ...
            {
                "opportunity": { "title": "Strategic Opportunities", "summary": "...", "relevance": 1-10, "ids": [numbers] },
                "urgent": { "title": "Critical / Urgent", "summary": "...", "relevance": 1-10, "ids": [numbers] },
                "lead": { "title": "Growth & Leads", "summary": "...", "relevance": 1-10, "ids": [numbers] },
                "risk": { "title": "Operational Risks", "summary": "...", "relevance": 1-10, "ids": [numbers] },
                "follow_up": { "title": "Pending Follow-ups", "summary": "...", "relevance": 1-10, "ids": [numbers] },
                "important": { "title": "Key Updates", "summary": "...", "relevance": 1-10, "ids": [numbers] }
            }

            IMPORTANT: Use the numerical ID from the label (e.g., if you identify 'Email 5', return 5 in the ids array). These IDs are global (1-50).
            ...
            try {
                ...
                const response = await this.callOpenRouter(messages, { 
                    ...
                    maxTokens: 2000, 
                    privacyMode,
                    // Use Qwen or Gemma for fastest response on small chunks
        ...
        batchResults.forEach((batchRes, batchIdx) => {
            ...
            categories.forEach(cat => {
                if (batchRes[cat]) {
                    ...
                    if (res.summary && res.summary.length > 20) {
                        result[cat].summary = (result[cat].summary === 'Reviewing inbox for relevant items...') ? res.summary : result[cat].summary + " " + res.summary;
...
Do not include any text before or after the JSON. Do not use code blocks.`
    ...
    async generateDraftReplyStream(emailContent, category, userContext = {}, privacyMode = false) {
        ...
        return await this.callOpenRouterStream([
            { role: 'system', content: systemPrompt },
            { role: 'user', content: `Email to reply to:\n\n${cleanContent}\n\nWrite the reply now.` }
        ], { temperature: useVoiceCloning ? 0.4 : 0.5, privacyMode });
    }
    ...
    /**
     * Internal method to build the draft reply system prompt
     */
    getDraftReplyPrompt(userName, fromName, roleInfo, goalsInfo, toneInstructions, securityInstruction, voicePrompt, useVoiceCloning, tone) {
        if (useVoiceCloning) {
            return `You are **${userName}**, an elite founder. ${roleInfo}${goalsInfo}\n${voicePrompt}\n\nRules:\n1. Think in <think></think>.\n2. Draft in <email></email>.\n3. Greet ${fromName}.\n4. Sign off with ${userName}.\n${securityInstruction}`;
        } else {
