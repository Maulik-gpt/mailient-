...
6. CRITICAL: Do NOT echo back dates, sender names, or technical timestamps. Focus ONLY on the core message and its significance.`
    ...
    async generateDraftReply(emailContent, category, userContext = {}, privacyMode = false) {
        ...
        try {
            ...
            let systemPrompt = '';
            if (useVoiceCloning) {
                systemPrompt = `You are ${userName} writing an email reply to ${fromName}. Mimic ${userName}'s exact writing style from the voice profile below. ${roleInfo}${goalsInfo}
...
INSTRUCTIONS:
1. First, think and reason internally about what ${userName} would write. Consider the voice profile, the email content, and the relationship.
2. Then write ONLY the final email reply. DO NOT output your thinking process, internal debate, or reasoning.
3. Never start with phrases like "We need to", "I should", "Given the", "However", "Let's see", or any meta-commentary.
4. NO em dashes (—). Use commas or periods only.
5. Maximum ${tone === 'concise' ? '50' : '120'} words.
6. Follow the voice profile greeting/sign-off patterns exactly.

OUTPUT ONLY THE FINAL EMAIL:${securityInstruction}`;
            } else {
...
INSTRUCTIONS:
1. First, think and reason internally about what ${userName} would write. Consider the tone, the email content, and the relationship.
2. Then write ONLY the final email reply. DO NOT output your thinking process, internal debate, or reasoning.
3. Never start with phrases like "We need to", "I should", "Given the", "However", or any meta-commentary.
