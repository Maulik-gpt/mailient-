...
export class OpenRouterAIService {
    constructor() {
        // Replace invalid hallucinated model strings with real OpenRouter models
        this.model = 'nvidia/llama-3.1-nemotron-70b-instruct:free';
        this.fallbackModel = 'google/gemini-2.0-flash-lite-preview-02-05:free';
        
        // Key Pool for 3-key rotation to prevent 400 errors and rate limits
        this.apiKeys = [
        ...
        // Specialized configurations for various features
        this.specializedModel = this.model;
        this.voiceCloningModel = 'meta-llama/llama-3.2-3b-instruct:free'; // Fast model for style matching

        if (this.apiKeys.length === 0) {
            console.error('❌ CRITICAL: No OPENROUTER_API_KEYs found in environment variables');
        } else {
            console.log(`🔑 AI Service initialized with ${this.apiKeys.length} keys in rotation using ${this.model}`);
        }
    }
    ...
    async callOpenRouter(messages, options = {}) {
        ...
        console.log('🤖 AI Request started. Key Pool Size: ' + this.apiKeys.length);

        // Try alternating keys if the first one fails
        for (let attempt = 0; attempt < this.apiKeys.length; attempt++) {
            const apiKey = this.getNextKey();
            
            for (const modelId of models) {
                try {
                    console.log(`📡 Attempting ${modelId} with key #${this.currentKeyIndex}`);
                    ...
                    const response = await fetch(`${this.baseURL}/chat/completions`, {
                        ...
                        body: JSON.stringify({
                            ...
                            max_tokens: options.maxTokens || 1000, // Increased for Nemotron
                            provider: {
                    ...
                    if (!response.ok) {
                        ...
                        console.warn(`⚠️ ${modelId} failed (${response.status}): ${errorText.substring(0, 100)}`);
        ...
        throw new Error('All OpenRouter keys and models failed. Check billing or API status.');
    }

    async callOpenRouterWithModel(messages, options = {}, model = null) {
        // Reuse the robust callOpenRouter with a specific override if needed
...
5. Use a warm, authoritative tone.`
            ...
            console.log('📝 Generating email summary with Specialized Model...');
            ...
            // Throw so the API route can return the real error to the client
...
**TONE ADJUSTMENT**: Even while mimicking their style, apply a **${tone}** slant to this specific message: ${toneInstructions}
...
6. **CONCISE**: Maximum ${tone === 'concise' ? '50' : '120'} words.

**OUTPUT FORMAT**:
- Greeting to ${fromName} (e.g., "Hi ${fromName},")
- Body of the reply (responding to what ${fromName} wrote)
- Sign-off with ${userName}'s name${securityInstruction}`;
            } else {
                // Standard draft with tone support
                systemPrompt = `You are ARCUS, drafting an email reply on behalf of ${userName}. ${roleInfo}${goalsInfo}
...
6. **CONCISE & PUNCHY**: Maximum ${tone === 'concise' ? '50' : '120'} words. 
...
**OUTPUT FORMAT** (JSON only - MAKE IT SPECIFIC AND ACTIONABLE):
{
    "opportunity": { 
        "title": "Opportunities", 
        "summary": "Company X wants to discuss $50k partnership - Reply by Friday. Investor Y interested in Series A - Schedule call next week", 
        "relevance": 8, 
        "ids": [0, 2, 5] 
    },
    "urgent": { 
        "title": "Urgent Actions", 
        "summary": "Client contract expires TOMORROW - Sign immediately. Server downtime reported - Contact IT team ASAP", 
        "relevance": 9, 
        "ids": [1, 3] 
    },
    "lead": { 
        "title": "New Leads", 
        "summary": "3 new demo requests from Fortune 500 companies. Startup founder reached out for enterprise pricing", 
        "relevance": 7, 
        "ids": [4, 6] 
    },
    "risk": { 
        "title": "Issues Requiring Attention", 
        "summary": "Customer complaint about delayed shipment - Offer refund ASAP. Payment gateway integration broken - Fix before billing cycle", 
        "relevance": 8, 
        "ids": [7, 8, 12] 
    },
    "follow_up": { 
        "title": "Follow-ups Needed", 
        "summary": "Reply to John's proposal questions. Schedule meeting with marketing team about campaign", 
        "relevance": 5, 
        "ids": [10, 13] 
    },
    "important": { 
        "title": "Important Updates", 
        "summary": "Q3 financial reports available. New team member starts Monday. Office closed for holidays", 
        "relevance": 4, 
        "ids": [17, 19] 
    }
}
