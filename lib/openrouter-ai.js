...
export class OpenRouterAIService {
    ...
    async generateInboxIntelligence(gmailData, privacyMode = false) {
        ...
        try {
            ...
            return await Promise.race([
                (async () => {
                    const batchSize = 10; // Process 10 emails per batch for optimal speed
                    const batchPromises = [];

                    for (let i = 0; i < totalEmails.length; i += batchSize) {
                        const batch = totalEmails.slice(i, i + batchSize);
                        const batchIds = batch.map(e => e.id);
                        // Remove delays for speed - process all batches in parallel
    ...
    async _processIntelligenceBatch(emails, emailIds, categories, privacyMode) {
        const emailList = emails.map((e, idx) => {
            const content = (e.body || e.snippet || '').substring(0, 150); // 150 chars max for speed
            return `${idx + 1}. From: ${e.from} | Subj: ${e.subject} | ${content}`;
        }).join('\n');

        const prompt = `Analyze each email and categorize.

Categories: ${categories.join(', ')}

For emails that match a category, output JSON:
{"opportunity": {"summary": "title", "description": "why", "relevance": 8, "ids": ["1"]}}

If no emails match a category, don't include it.
...
JSON:`;
        ...
        try {
            const response = await this.callOpenRouter([{ role: 'user', content: prompt }], {
                ...
                privacyMode,
                model: 'openrouter/free',
                timeout: 6000, // 6s hard limit to stay under 10s total
                singleModel: true // No fallback retries to ensure we stay under 10s
            });

            const content = this.extractResponse(response);
            console.log(`🤖 [Sift AI] AI response: ${content?.substring(0, 300)}...`);
            const aiResult = this._safeParseJSON(content);
            console.log(`📊 [Sift AI] Parsed result:`, Object.keys(aiResult));
            ...
            console.error(`❌ [Sift AI] Batch failed: ${e.message}`);
            console.error(`❌ [Sift AI] Prompt was: ${prompt.substring(0, 200)}...`);
            console.error(`❌ [Sift AI] Response was: ${content?.substring(0, 200)}...`);
            return null;
        }
    }
