...
export class OpenRouterAIService {
    ...
    constructor() {
        // Standardized strictly on fast free models to guarantee <5s responses and prevent 504 timeouts
        this.model = 'google/gemini-2.0-flash-lite-preview-02-05:free';
        this.fallbackModels = ['meta-llama/llama-3.3-70b-instruct:free', 'openrouter/free'];
        ...
        // All features use the standardized fast free model
        this.specializedModel = 'google/gemini-2.0-flash-lite-preview-02-05:free';
        this.voiceCloningModel = 'google/gemini-2.0-flash-lite-preview-02-05:free';
...
After the </think> tag, output ONLY the final 2-3 sentence summary. No JSON, no labels, no formatting in the final output.`
    ...
    async generateInboxIntelligence(gmailData, privacyMode = false) {
        ...
        try {
            ...
            // Run batches sequentially to avoid OpenRouter free tier concurrency limits (429)
            // which lead to 5000ms retry loops that break the 20-second speed requirement.
            for (let i = 0; i < batches.length; i++) {
                const batch = batches[i];
                const batchNum = i + 1;
                console.log(`📦 [Sift AI] Processing batch ${batchNum}/${batches.length}...`);
                
                let result = null;
                let retries = 0;
                const MAX_RETRIES = 1; // Reduce retries to fail faster if needed
                ...
                while (retries <= MAX_RETRIES) {
                    ...
                    retries++;
                    if (retries <= MAX_RETRIES) await new Promise(resolve => setTimeout(resolve, 3000));
                }
            }
            ...
            try {
                ...
                if (Array.isArray(parsedJSON)) {
                    results = parsedJSON.map(item => {
                        ...
                        else if (category.includes('news')) category = 'newsletters';
                ...
                const segments = rawContent.split(/["']?id["']?\s*:\s*["']/i);
