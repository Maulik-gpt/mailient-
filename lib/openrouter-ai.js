...
OUTPUT FORMAT: You MUST wrap your final email body in <email> and </email> tags. Any thinking must happen outside these tags. Example: <email>Hey there, thanks!</email>`;
            ...
            // If the model included <email> tags, extract only what's inside
            if (draft && draft.includes('<email>') && draft.includes('</email>')) {
                draft = draft.substring(draft.indexOf('<email>') + 7, draft.lastIndexOf('</email>')).trim();
            } else if (draft && draft.includes('</think>')) {
                // Also strip standard think tags if present
...
OUTPUT FORMAT: You MUST wrap your final email body in <email> and </email> tags. Any thinking must happen outside these tags. Example: <email>Hey there, thanks for letting us know!</email>`;
            ...
            let repairReply = this.extractResponse(response);
            
            // If the model included <email> tags, extract only what's inside
            if (repairReply && repairReply.includes('<email>') && repairReply.includes('</email>')) {
                repairReply = repairReply.substring(repairReply.indexOf('<email>') + 7, repairReply.lastIndexOf('</email>')).trim();
            } else if (repairReply && repairReply.includes('</think>')) {
                repairReply = repairReply.split('</think>').pop().trim();
            }
...
OUTPUT FORMAT: You MUST wrap your final email body in <email> and </email> tags. Any thinking must happen outside these tags. Example: <email>Hi there, just following up!</email>`;
            ...
            let followUp = this.extractResponse(response);
            
            // If the model included <email> tags, extract only what's inside
            if (followUp && followUp.includes('<email>') && followUp.includes('</email>')) {
                followUp = followUp.substring(followUp.indexOf('<email>') + 7, followUp.lastIndexOf('</email>')).trim();
            } else if (followUp && followUp.includes('</think>')) {
                followUp = followUp.split('</think>').pop().trim();
            }
