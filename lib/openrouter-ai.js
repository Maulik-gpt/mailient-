...
export class OpenRouterAIService {
    ...
    async _processIntelligenceBatch(emails, emailIds, categories, privacyMode) {
        ...
        const prompt = `Analyze these ${emails.length} emails. Categorize into: ${categories.join(', ')}.
Output ONLY JSON: {"category": {"summary": "One sentence summary", "description": "2-3 sentence logical explanation", "relevance": 1-10, "ids": ["ID_NUMBER"]}}
...
${emailList}`;
        ...
        try {
            const response = await this.callOpenRouter([{ role: 'user', content: prompt }], {
                ...
                maxTokens: 1000,
                privacyMode,
                model: 'google/gemma-7b-it:free', // Ultra-stable 7B model for reliable extraction
                timeout: 10000, 
                singleModel: true 
            });
