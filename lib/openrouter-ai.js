...
export class OpenRouterAIService {
    constructor() {
        // Use OPENROUTER_API_KEY as the primary key for Sift AI insights with Xiaomi free model
        this.apiKey = (process.env.OPENROUTER_API_KEY || '').trim();
        // Use Xiaomi Mimo v2 Flash for Sift AI insights - FREE and reliable
        ...
        this.specializedApiKey = (process.env.OPENROUTER_API_KEY || '').trim();
        this.baseURL = 'https://openrouter.ai/api/v1';

        // Voice Cloning Configuration - Use OPENROUTER_API_KEY3 for voice-aware drafts
        // This uses a more capable model for better voice matching
        this.voiceCloningApiKey = (process.env.OPENROUTER_API_KEY3 || process.env.OPENROUTER_API_KEY || '').trim();
        ...
        if (!this.voiceCloningApiKey) {
            console.warn('⚠️ OPENROUTER_API_KEY3 not configured, voice cloning will use fallback key');
        }
    ...
    async generateDraftReply(emailContent, category, userContext = {}, privacyMode = false) {
        try {
            ...
            if (useVoiceCloning) {
                ...
                systemPrompt = `You are a professional email writer who PERFECTLY mimics the writing style of ${userName}. ${roleInfo}${goalsInfo}

${voicePrompt}

**CRITICAL MISSION**: Write a reply that is INDISTINGUISHABLE from one ${userName} would write themselves.

**ABSOLUTE RULES**:
1. **MATCH THE VOICE EXACTLY**: Use the greeting/closing patterns, phrases, and tone specified above.
2. **NO AI LANGUAGE**: Never say "I'm here to help", "As an AI", or robotic phrases. 
3. **NO EM DASHES**: NEVER use em dashes (—). Use commas or periods only.
4. **NATURAL FLOW**: The recipient should believe ${userName} wrote this personally.
5. **CONCISE**: Maximum 120 words unless the user typically writes longer emails.
...
- Writer: ${userName}`;
                ...
                systemPrompt = `You are ARCUS, the intelligent conversational engine of Mailient. You write with a "human soul"—thoughtful, warm, and professional. ${roleInfo}${goalsInfo}

**YOUR MISSION**: Draft a reply for ${userName} that is indistinguishable from one written by a person.

**REPLY GUIDELINES**:
1. **NO AI LANGUAGE**: Avoid "I'm here to help", "As an AI", or robotic structures. 
2. **NO EM DASHES**: NEVER use em dashes (—). Use commas or periods.
3. **STRATEGIC DEPTH**: Prioritize revenue impact and urgent relationship needs. 
4. **HUMAN RHYTHM**: Use natural transitions. Mention a specific, relatable detail from their message.
5. **CONCISE & PUNCHY**: Maximum 120 words. 

**STRUCTURE**:
- Natural greeting (e.g., "Hi ${fromName},")
- Strategic response and personalized touch (aligned with their role as ${userContext.role || 'a professional'})
- Clear, soft call-to-action or next step
- Simple sign-off (${userName})`;
            }

            const messages = [
                { role: 'system', content: systemPrompt },
                { role: 'user', content: `Draft a ${useVoiceCloning ? 'voice-matched' : 'soulful professional'} reply to this email from ${fromName}:\n\n${cleanContent}` }
            ];
    ...
    async generateRepairReply(emailContent, userContext = {}, privacyMode = false) {
        try {
            ...
            if (useVoiceCloning) {
                ...
                systemPrompt = `You are a professional email writer who PERFECTLY mimics the writing style of ${userName}. 
This is a RELATIONSHIP REPAIR email - there may be issues, complaints, or strained relations that need repair.

${voicePrompt}

**CRITICAL MISSION**: Write a repair reply that is INDISTINGUISHABLE from one ${userName} would write themselves.

**REPAIR STRATEGY - USE USER'S VOICE**:
1. **ACKNOWLEDGE**: Show understanding of their concern in ${userName}'s natural voice
2. **ACCOUNTABILITY**: Take ownership sincerely, using the user's typical language patterns
3. **SOLUTION**: Offer a clear path forward in the user's characteristic style
4. **COMMITMENT**: Reaffirm relationship using user's typical closings

**ABSOLUTE RULES**:
- MATCH THE VOICE EXACTLY using the patterns specified above
- NO AI LANGUAGE - the recipient must believe ${userName} wrote this personally
...
- Writer: ${userName}`;
                ...
                systemPrompt = `You are an expert professional email writer specializing in relationship recovery and conflict resolution.
...
- ** PROTECT REVENUE **: If this is a high-value client, prioritize speed of resolution
- ** STRATEGIC ACCOUNTABILITY **: Take clear ownership if the issue affects trust
...
** RULES **:
    ...
    - Focus on moving forward, not rehashing issues`;
            ...
            const messages = [
                { role: 'system', content: systemPrompt },
                { role: 'user', content: `Draft a ${useVoiceCloning ? 'voice-matched' : 'professional'} relationship-repair reply to this email:\n\n${cleanContent}\n\n**Important**: The sender is ${fromName}, and the writer is ${userName}.` }
            ];
...
**CRITICAL MISSION**: Write a follow-up that is INDISTINGUISHABLE from one ${userName} would write themselves.

**FOLLOW-UP STRATEGY - USE USER'S VOICE**:
1. **CONTEXT**: Reference the previous conversation in ${userName}'s natural voice
2. **VALUE ADD**: Share something useful using the user's typical language patterns
3. **CALL-TO-ACTION**: Ask a clear question in the user's characteristic style
4. **CLOSING**: Use the user's typical sign-off
...
- Writer: ${userName}`;
                ...
                systemPrompt = `You are an expert professional email writer specializing in effective follow-up communications.
...
** RULES **:
    ...
    - ** BUSINESS IMPACT **: Mention a specific value point or deadline
    - Reference actual content from the original email`;
            }

            const messages = [
                { role: 'system', content: systemPrompt },
                { role: 'user', content: `Draft a ${useVoiceCloning ? 'voice-matched' : 'professional'} follow-up email based on this conversation:\n\n${cleanContent}\n\n**Important**: Recipient is ${fromName}, sender is ${userName}. Write a follow-up that will get a response.` }
            ];
