...
export class OpenRouterAIService {
    ...
    async generateInboxIntelligence(gmailData, privacyMode = false) {
        ...
        // Phase 1: High-Speed Batching - limit to 20 emails for < 10s response
        ...
        try {
            // Safety: Race all batches with a strict 9.5s timeout to guarantee < 10s response
            const BATCH_TIMEOUT = 9500;
    ...
    async _processIntelligenceBatch(emails, emailIds, categories, privacyMode) {
        const emailList = emails.map((e, idx) => {
            const content = (e.body || e.snippet || '').substring(0, 200); // 200 chars max per email
...
${emailList}`;
        ...
        try {
            const response = await this.callOpenRouter([{ role: 'user', content: prompt }], {
                ...
                privacyMode,
                model: 'openrouter/free',
                timeout: 8000, // 8s hard limit for the API call
                singleModel: true // No fallback retries to ensure we stay under 10s
            });
