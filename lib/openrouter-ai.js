...
export class OpenRouterAIService {
    ...
    async callOpenRouterStream(messages, options = {}) {
        ...
        for (let keyAttempt = 0; keyAttempt < keys.length; keyAttempt++) {
            ...
            if (!response.ok) {
                ...
                if (errorStatus === 429) {
                    console.warn(`⏳ OpenRouter stream rate limited (429) on ${modelId}. Failing for fallback...`);
                    throw new Error(`MODEL_RATE_LIMITED: ${modelId}`);
                }
                                    ...
                                    if (content) {
                                        ...
                                        if (!isInsideThink) {
                                            if (isInsideEmail) {
                                                ...
                                                // Filter out common instruction-repeating prefixes in fallback mode
                                                ...
                                                for (const prefix of monologuePrefixes) {
                                                    if (prefix.test(cleanBuffer)) {
                                                        cleanBuffer = cleanBuffer.replace(prefix, '').trim();
                                                        // If we matched a prefix, we wait for more content
                                                        buffer = cleanBuffer;
                                                        continue; 
                                                    }
                                                }
                                            ...
                                            // We are inside <think>, just keep accumulating in buffer 
                                            // so we can find the closing tag.
                                            // To prevent memory leaks on very long thinking, we keep it small.
    ...
    extractResponse(data) {
        ...
        if (content) {
            // Strip out <think> blocks (Case-insensitive to be safe)
            content = content.replace(/<think>[\s\S]*?<\/think>/gi, '').trim();
            
            // Strip common "monologue" prefixes if the AI leaks them
            const prefixesToRemove = [
                /^We need to produce a Cognitive Synthesis/i,
                /^Based on the email/i,
                /^Here is the/i,
                /^Sure, I can help/i,
                /^I have analyzed/i,
                /^Synthesize this email/i
            ];
            ...
            // Remove leading colons or artifacts left over from prefix removal
    ...
    extractEmailDraft(rawResponse) {
        ...
        // Strip out deepseek/claude/qwen style thinking/reasoning blocks
        draft = draft.replace(/<think(ing)?>[\s\S]*?<\/think(ing)?>/gi, '');
        
        // Strip tool call tags that sometimes leak from models (OpenRouter/Qwen/Nemotron)
    ...
    async generateEmailSummary(emailContent, privacyMode = false, userContext = {}, options = {}) {
        ...
        try {
            ...
            const cleanContent = emailContent.substring(0, 3000).trim();
...
RULES:
1. Think inside <think></think> tags first (hidden from user).
2. Provide the final synthesis DIRECTLY after the think block.
3. Be profoundly insightful and concise.
4. Identify the hidden intent and the REVENUE or STRATEGIC impact relevant to their role/goals.
5. Keep it to 3-4 sentences that sound human, not like an AI summary.
6. NEVER use em dashes (—).
7. Use a warm, authoritative tone.
8. CRITICAL: Do NOT echo back dates, sender names, or technical timestamps. Focus ONLY on the core message and its significance.
9. CRITICAL: Deliver exactly 3-4 sentences of deep insight. No more, no less.
10. DO NOT repeat the instructions, DO NOT state your understanding of the task, and DO NOT speak to the user.
11. Start the final output DIRECTLY after the </think> tag. If no <think> tag is used, output ONLY the 3-4 sentences synthesis.`
                },
            ...
            const response = await this.callOpenRouterWithModel(messages, { 
                maxTokens: 150, // Reduced from 400 for speed
                temperature: 0.3, 
                privacyMode, 
                model: 'openrouter/free', // Use auto-routed free model for fastest and guaranteed free response
                ...options 
            });
...
`;
    ...
    async generateInboxIntelligence(gmailData, privacyMode = false) {
        // Fetch latest 45 emails as requested for speed and accuracy
        const emails = gmailData.slice(0, 45);
        ...
        console.log(`🚀 [TurboSift] Analyzing latest ${emails.length} emails using Gemini 2.0 Flash (Sub-10s Mission)...`);
        
        const emailList = emails.map((e, idx) =>
            `ID: ${idx + 1} | From: ${e.from} | Subject: ${e.subject} | Content: ${(e.body || e.snippet || '').substring(0, 700)}\n`
        ).join('\n---\n');

        const prompt = `You are a high-level Strategic Intelligence Analyst for Mailient. Analyze the following 50 emails and identify key business signals across categories.
        
        OUTPUT JSON SCHEMA (Strict - Use EXACTLY these keys):
        {
            "opportunity": { "title": "Strategic Opportunities", "summary": "Concise summary", "description": "Specific strategic context (30-50 words)", "relevance": 1-10, "ids": [IDs] },
            "urgent": { "title": "Critical / Urgent", "summary": "Concise summary", "description": "Why this is urgent and what is blocked (30-50 words)", "relevance": 1-10, "ids": [IDs] },
            "lead": { "title": "Growth & Leads", "summary": "Concise summary", "description": "Lead quality and potential value (30-50 words)", "relevance": 1-10, "ids": [IDs] },
            "risk": { "title": "Operational Risks", "summary": "Concise summary", "description": "Nature of risk and potential fallout (30-50 words)", "relevance": 1-10, "ids": [IDs] },
            "follow_up": { "title": "Pending Follow-ups", "summary": "Concise summary", "description": "Status of conversation and next move (30-50 words)", "relevance": 1-10, "ids": [IDs] },
            "important": { "title": "Key Updates", "summary": "Concise summary", "description": "Significance of this update (30-50 words)", "relevance": 1-10, "ids": [IDs] }
        }

        CRITICAL CONSTRAINTS: 
        1. Descriptions MUST be insightful, premium, and data-driven. NO generic filler.
        2. Use the numerical ID (1-50) from the list below.
        3. If no items match a category, return empty arrays/null for that category.
        4. Focus on the most RECENT and SIGNIFICANT items.
        ...
        try {
            // Use fast auto-routed model for ultra-low latency
            // PASS TIMEOUT and BYPASS CHAIN for Sift AI to ensure <10s response
            const response = await this.callOpenRouter(messages, { 
                temperature: 0.1, 
                maxTokens: 2000, 
                privacyMode,
                model: 'openrouter/free',
                timeout: 30000, // Increased to 30s for complex inbox analysis
                singleModel: false // Allow fallback
            });
            ...
            console.error(`❌ TurboSift Analysis failed:`, error.message);
            // Fallback will be handled by createFallbackResponse + local rules
        }
        ...
        categories.forEach(cat => {
            // Flexible key matching (handles "opportunity_signals", "opportunitySignals", etc.)
            const possibleKeys = [cat, cat + '_signals', cat + 'Signals', cat.replace('_', '')];
            let res = null;
            ...
            if (res && res.summary && res.summary.length > 5) {
                result[cat].summary = res.summary.trim().replace(/^[-*]\s*/, '');
                result[cat].relevance = Math.max(1, Math.min(10, res.relevance || 5));
                result[cat].description = res.description || '';
                
                if (Array.isArray(res.ids)) {
                    res.ids.forEach(id => {
                        const numericId = parseInt(id);
                        if (!isNaN(numericId) && numericId > 0 && numericId <= emails.length) {
                            const actualId = emailIds[numericId - 1];
        ...
        // Run local rules for any remaining gaps (ensures we always have content)
    ...
    buildEnhancedResponse(result, emailIds, categories) {
        const insights = categories.map(category => {
            ...
            // Use AI-generated description as primary content, fallback to summary or generic phrases
            let enhancedContent = data.description || data.summary;

            if (!enhancedContent || enhancedContent === 'Identified items' || enhancedContent === 'Reviewing inbox for relevant items...') {
                if (ids.length > 0) {
                    const categoryPhrases = {
                        opportunity: `Analyzed communication patterns suggesting ${ids.length} potential strategic growth areas or partnership opportunities.`,
                        urgent: `Detected ${ids.length} time-sensitive items requiring prompt attention based on deadline or priority signals.`,
                        lead: `Identified ${ids.length} potential business inquiries or growth signals for further exploration.`,
                        risk: `Flagged ${ids.length} items with signals suggesting operational or relationship risks.`,
                        follow_up: `Noted ${ids.length} active conversation threads that may require continuation.`,
                        important: `Categorized ${ids.length} relevant updates or significant informational items.`
                    };
                    enhancedContent = categoryPhrases[category] || `${ids.length} relevant items identified through context analysis.`;
                } else {
                    enhancedContent = 'No significant items identified in this category from your current inbox data.';
                }
            }
    ...
    async generateChatResponse(userMessage, emailContext = null, privacyMode = false) {
        try {
            const systemContent = `You are Sift AI, an advanced executive assistant with direct, secure access to the user's Gmail inbox. You have the ability to read, search, and analyze their emails. NEVER state that you cannot access their emails or internal data. If you need to see their emails to answer a question, guide them to say "check my inbox" or "read my unread emails".` + (emailContext ? '\n\nContext from inbox:\n' + emailContext : '') + (privacyMode ? ' [PRIVACY MODE ENABLED: Do not use this conversation for training.]' : '');
            const messages = [
...
1. Think inside <think></think> tags (hidden).
2. Write the final note following this structure:
...
TONE: Professional, insightful, and extremely concise. Keep total response under 150 words.
DO NOT repeat the instructions, speak to the user, or explain your reasoning outside the think tags.
If no <think> tags are used, output ONLY the structured note.`
                },
...
6. Length: Aim for a medium-sized, thoughtful reply (80-150 words) unless the voice profile explicitly demands brevity or the user chose "concise" tone.
7. Sound human, not like an AI. Be proactive: if a meeting or task is implied, mention you can handle it.${securityInstruction}
8. FORMATTING: Use bold (<b>) for key commitments or dates. Use bullet points for lists. Use <a> tags for links. Return RICH HTML.
9. DO NOT include any tool calling tags like <tool_call>, <arg_value>, or json-like structures in your final output. ONLY provide the thinking and the email.
10. CRITICAL: If any 'Custom instructions' or '[CRITICAL DIRECTIVE]' are provided in the context, you MUST follow them strictly after understanding the email context. They are the absolute highest priority and override all other suggestions.
11. DO NOT speak to the user or repeat instructions. The content between <email> and </email> must be ONLY the email message ready to be sent.`
