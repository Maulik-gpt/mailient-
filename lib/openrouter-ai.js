...
export class OpenRouterAIService {
    ...
    extractResponse(data) {
        ...
        if (content) {
            ...
            // Strip JSON wrappers: free models sometimes wrap output in {"result": "..."}
            content = content.replace(/```json\n?/gi, '').replace(/```\n?/g, '').trim();
            const jsonWrapperMatch = content.match(/^\s*\{\s*"(?:result|summary|response|output|text|content|answer)"\s*:\s*"([\s\S]*?)"\s*\}\s*$/i);
            if (jsonWrapperMatch) {
                content = jsonWrapperMatch[1].replace(/\\n/g, '\n').replace(/\\"/g, '"');
            }
            ...
            // Strip leading labels like "Summary:" when the model echoes the instruction
    ...
    async generateEmailSummary(emailContent, privacyMode = false, userContext = {}, options = {}) {
        ...
        try {
            ...
            const goalContext = userContext.goals?.length > 0 ? `Goals: ${userContext.goals.join(', ')}. ` : '';

            const messages = [
                {
                    role: 'system',
                    content: `You are a premium executive email analyst. ${roleContext}${goalContext}\nProvide a detailed, well-structured intelligence briefing of the email. Include:\n- Who sent it and their intent\n- Key points and important details\n- Any action items or deadlines\n- Strategic context or implications\nWrite in clear, professional prose. Use multiple paragraphs. Do NOT wrap your response in JSON. Do NOT use labels like "Summary:". Just write the analysis directly.`
                },
                { role: 'user', content: `Analyze this email:\n\n${cleanContent}` }
            ];

            const response = await this.callOpenRouterWithModel(messages, { maxTokens: 600, temperature: 0.3, privacyMode, ...options });
            return this.extractResponse(response) || this.generateBasicSummary(emailContent);
        } catch (error) {
    ...
    async generateDraftReply(emailContent, category, userContext = {}, privacyMode = false, options = {}) {
        ...
        try {
            ...
            const response = await this.callOpenRouterWithModel(messages, { maxTokens: 600, temperature: useVoiceCloning ? 0.4 : 0.5, privacyMode, ...options });
    ...
    generateVoicePrompt(voiceProfile) {
        ...
        return `VOICE PROFILE:\n- Style: ${tone?.primary || 'standard'}\n- Greetings: ${greeting_patterns?.preferred_greetings?.join(', ')}\n- Sign-offs: ${closing_patterns?.preferred_closings?.join(', ')}\n- Language: ${language_patterns?.sentence_length} sentences, ${language_patterns?.vocabulary_complexity} vocab.\n- Structure: ${structural_patterns?.typical_email_length}.`;
    }

    async generateInboxIntelligence(gmailData, privacyMode = false) {
        // Standardize Sift AI retrieval with firm 45-item limit
        ...
        const prompt = `Analyst. Analyze these emails. Categorize into: ${categories.join(', ')}.
Return ONLY valid JSON. Each category needs: summary (1 sentence), description (2-3 sentence plain text overview), relevance (1-10), ids (array of matching email ID numbers as strings).
Example: {"opportunity":{"summary":"...","description":"...","relevance":8,"ids":["1","3"]}}
Include ALL 6 categories. Empty categories get empty ids array. NO markdown, NO code blocks, ONLY the JSON object.
...
EMAILS:\n${emailList}`;
    ...
    getDraftReplyPrompt(userName, fromName, roleInfo, goalsInfo, toneInstructions, securityInstruction, voicePrompt, useVoiceCloning, tone) {
        return `Ghostwriter for ${userName}. Role: ${roleInfo}. Goals: ${goalsInfo}\n${voicePrompt}\n${toneInstructions}\n\nRULES:\n- Think in <think>.\n- Reply in <email>.\n- Greet ${fromName}. Sign ${userName}.\n- ${securityInstruction}`;
    }
