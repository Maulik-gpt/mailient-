...
export class OpenRouterAIService {
    ...
    extractResponse(data) {
        ...
        if (content) {
            // Architecturally correct: Clean thinking blocks and surrounding artifacts
    ...
    async generateEmailSummary(emailContent, privacyMode = false, userContext = {}, options = {}) {
        ...
        try {
            ...
            const messages = [
                {
                    role: 'system',
                    content: `SYSTEM: Elite Executive Email Analyst.
USER CONTEXT: ${roleContext}${goalContext}
OBJECTIVE: Provide a 3-4 sentence "Cognitive Synthesis" focusing on intent and strategic impact.

CONSTRAINTS:
- No preamble, meta-talk, or instruction repetition.
- No dates or technical timestamps.
- Warm, authoritative tone.
- Think inside <think> tags first.`
                },
                { role: 'user', content: `Summarize/Synthesize:\n\n${cleanContent}` }
            ];
    ...
    async generateNote(emailContext, category = 'general', privacyMode = false) {
        try {
            ...
            const messages = [
                {
                    role: 'system',
                    content: `SYSTEM: Executive Assistant (Sift AI).
OBJECTIVE: Generate a high-fidelity, outcome-focused note for a "${category}" email.
PRIVACY: ${privacyMode ? 'ENABLED' : 'DISABLED'}

STRUCTURE:
- **Summary**: Outcome and Intent (1-2 sentences).
- **Key Points**: FACTUAL (detail) and RELATIONAL (sentiment).
- **Action Items**: Explicit system state or task.
- **Context**: Strict, system-level event description.
...
- NO meta-talk.`
            ...
            console.log(`📝 Generating ${category} note...`);
            const response = await this.callOpenRouterWithModel(messages, { maxTokens: 400, temperature: 0.3, privacyMode }, null, null);
...
RULES:
- Think inside <think> tags.
- Output ONLY the email inside <email> tags.
- Greet ${fromName} by name. Sign as ${userName}.
- Length: 80-150 words (unless concise).
- No em dashes. No meta-talk.
- RICH HTML (<b> for dates/commits, <a> for links).
- ${securityInstruction}
- CRITICAL: Follow [CRITICAL DIRECTIVE] if provided.`;
