...
export class OpenRouterAIService {
    // apiKey -> blockedUntil (ms epoch). The free-models-per-day quota resets at
    // 00:00 UTC, so a quota-exhausted key is blocked ONLY until then and then
    // auto-recovers. The old `new Set()` never cleared, so once a key 429'd it
    // stayed blocked in memory forever (warm serverless instance) — which is why
    // the pool stayed "rate-limited" long after OpenRouter's daily reset.
    static blockedKeys = new Map();

    static blockKey(key) {
        const now = new Date();
        const nextResetUtc = Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate() + 1, 0, 0, 0);
        OpenRouterAIService.blockedKeys.set(key, nextResetUtc);
    }
    ...
    // Live-verified free model ids (2026-06). PRIMARY is now NVIDIA Nemotron 3
    // Ultra (550B-A55B, 1M ctx, tool-capable) — strongest free model on
    // OpenRouter. gpt-oss-120b/20b retired (product decision); 'z-ai/glm-4.5-air'
    // removed (paid-only 404); 'openrouter/free' was never a real id.
    static FREE_MODELS = [
    ...
    // token, so drafts use the smaller fast free models (gemma-4-31b,
    // nemotron-nano-30b) which stream quickly and are plenty for a short
    // voice-matched reply. Used by generateDraftReplyStream.
    static DRAFT_MODELS = ['google/gemma-4-31b-it:free', 'google/gemma-4-26b-a4b-it:free', 'nvidia/nemotron-3-nano-30b-a3b:free'];

    // PAID FALLBACK models. These are appended to the END of the chain (never the
    // front), so they're only reached after every free model has been rate-limited
    // — free-first, paid-as-fallback. On by default so a rate-limited free pool
    // still returns a real answer; set DISABLE_PAID_FALLBACK=true for free-only.
    // Read at call time so env changes apply without a rebuild.
    static premiumModels() {
        if (process.env.DISABLE_PAID_FALLBACK === 'true') return [];
        const custom = (process.env.ARCUS_PREMIUM_MODELS || '').split(',').map(s => s.trim()).filter(Boolean);
        return custom.length ? custom : ['google/gemini-2.5-flash-lite', 'anthropic/claude-haiku-5', 'google/gemini-2.5-flash'];
    }
    ...
    async callOpenRouterStream(messages, options = {}) {
        ...
        for (let keyAttempt = 0; keyAttempt < keys.length; keyAttempt++) {
            ...
            if (!response.ok) {
                ...
                if (errorStatus === 429) {
                    if (errorText.includes('free-models-per-day') || errorText.includes('daily') || errorText.includes('limit exceeded')) {
                        console.warn(`🔒 [callOpenRouterStream] Key has exceeded its daily free quota. Blocking key...`);
                        OpenRouterAIService.blockKey(apiKey);
                    }
