...
export class OpenRouterAIService {
    ...
    async generateInboxIntelligence(gmailData, privacyMode = false) {
        ...
        // Phase 1: Parallel Batching - 4 batches of 10 emails each
        // This is the "Deep Fix" to handle 40 emails in <10s and avoid 504 timeouts.
        ...
        console.log(`📡 [Sift AI] Launching ${batchPromises.length} parallel batches for ${totalEmails.length} emails...`);
        const startTime = Date.now();

        try {
            // Race the batches with a hard 8.5s limit to avoid 504 gateway timeouts
            const batchResults = await Promise.all(batchPromises);
            const duration = (Date.now() - startTime) / 1000;
            console.log(`✅ [Sift AI] Parallel analysis complete in ${duration.toFixed(2)}s`);
            ...
            batchResults.forEach(batchResult => {
                ...
                categories.forEach(cat => {
                    ...
                    if (res?.summary && res.summary !== mergedResult[cat].summary) {
                        // Keep the best summary/description
                        if (mergedResult[cat].relevance < (res.relevance || 0)) {
            ...
            // Phase 3: Global Synthesis (Deep Logic)
            // Creates a high-level narrative across all categories
            ...
            console.error('🚀 [Sift AI] Parallel batching failed or timed out:', error.message);
            const emailIds = totalEmails.map(e => e.id);
