...
const TASK_REGISTRY = {
  email_reply: {
    ...
    schema: {
      fields: [
        ...
        { id: 'to', label: 'To', type: 'text', placeholder: 'recipient@email.com', required: true },
        ...
        { id: 'body', label: 'Body', type: 'textarea', placeholder: 'Draft email body', required: true },
  ...
  email_send: {
    ...
    schema: {
      fields: [
        { id: 'from', label: 'From', type: 'text', placeholder: 'your@email.com', required: true },
        { id: 'to', label: 'To', type: 'text', placeholder: 'recipient@email.com', required: true },
        { id: 'cc', label: 'Cc', type: 'text', placeholder: 'cc@email.com' },
        { id: 'bcc', label: 'Bcc', type: 'text', placeholder: 'bcc@email.com' },
        { id: 'subject', label: 'Subject', type: 'text', placeholder: 'Email subject', required: true },
  ...
  meeting_schedule: {
    ...
    schema: {
      fields: [
        ...
        { id: 'attendees', label: 'Attendees', type: 'textarea', placeholder: 'alice@org.com, bob@org.com', required: true },
        { id: 'day', label: 'Day', type: 'text', placeholder: 'Monday' },
        { id: 'date', label: 'Date', type: 'date', required: true },
        { id: 'time', label: 'Time', type: 'time', required: true },
        { id: 'timezone', label: 'Timezone', type: 'text', placeholder: 'Asia/Calcutta', required: true },
        { id: 'duration', label: 'Duration (minutes)', type: 'number', placeholder: '30' },
        { id: 'summary', label: 'Title', type: 'text', placeholder: 'Project sync', required: true },
  ...
  analytics_report: {
    ...
    schema: {
      fields: [
        { id: 'metrics', label: 'Metrics', type: 'textarea', placeholder: 'response_time, reply_rate, volume', required: true },
        { id: 'dateRange', label: 'Date Range', type: 'text', placeholder: 'last_30_days', required: true },
  ...
  summary: {
    ...
    schema: {
      fields: [
        { id: 'objective', label: 'Focus', type: 'text', placeholder: 'Inbox, project, or request to summarize' },
        { id: 'tone', label: 'Tone', type: 'text', placeholder: 'concise, executive, detailed' },
        { id: 'highlights', label: 'Key Highlights', type: 'textarea', placeholder: 'Top items to include' }
      ]
    },
  ...
  research: {
    ...
    schema: {
      fields: [
        { id: 'question', label: 'Research Question', type: 'text', placeholder: 'What should Arcus investigate?', required: true },
        { id: 'sources', label: 'Source Hints', type: 'textarea', placeholder: 'Links, teams, systems to check' },
        { id: 'deadline', label: 'Deadline', type: 'text', placeholder: 'today, tomorrow, specific date' }
      ]
    },
    actions: [{ actionType: 'apply_changes', label: 'Apply Changes', requiresApproval: false, autoExecute: true }]
  },
  action_plan: {
    ...
    schema: {
      fields: [
        { id: 'goal', label: 'Goal', type: 'text', placeholder: 'What outcome are we driving?', required: true },
        { id: 'steps', label: 'Steps', type: 'textarea', placeholder: 'One step per line with owners', required: true },
        { id: 'riskNotes', label: 'Risks / Dependencies', type: 'textarea', placeholder: 'Blocked calendars, missing data, approvals' }
      ]
    },
  ...
  generic_workflow: {
    ...
    schema: {
      fields: [
        { id: 'objective', label: 'Objective', type: 'text', placeholder: 'Describe what to execute', required: true },
        { id: 'inputs', label: 'Inputs', type: 'textarea', placeholder: 'List key inputs and constraints' },
        { id: 'steps', label: 'Steps', type: 'textarea', placeholder: 'One step per line', required: true }
      ]
    },
...
export function inferTaskType({ taskType = null, canvasType = 'none', intent = '', message = '' } = {}) {
  ...
  if (/(send (an )?email|compose|deliver)/.test(lower) || intent === 'send_email') return 'email_send';
  if (/(reply|respond)/.test(lower) || intent === 'reply_email' || intent === 'draft_email' || /(follow up)/.test(lower)) return 'email_reply';
  if (intent === 'summarize' || /summary|summarize/.test(lower)) return 'summary';
  if (/research|investigate|dig in|analysis/.test(lower) || intent === 'research') return 'research';
  if (/plan|action plan|steps/.test(lower) || intent === 'plan') return 'action_plan';
