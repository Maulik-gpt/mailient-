        ...
        <AnimatePresence mode="wait">
            ...
            <motion.div key="step1" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} transition={{ duration: 0.4 }} className="space-y-6">
              ...
              <form onSubmit={handleEmailSubmit} className="space-y-5">
                <div className="animate-element animate-delay-300">
                   <label className="text-[11px] font-bold text-white/20 uppercase tracking-widest mb-2 block">Work Email</label>
                   <GlassInputWrapper>
                    <input ref={emailInputRef} type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="Work email address" className="w-full bg-transparent text-sm p-4 rounded-2xl focus:outline-none text-white placeholder:text-white/10 font-medium" autoFocus required />
                  </GlassInputWrapper>
                </div>

                <div className="animate-element animate-delay-500 flex items-center justify-between text-[12px]">
                  <button type="button" onClick={() => router.push('/auth/signin')} className="text-white/20 hover:text-white transition-colors tracking-tight">
                    already registered? <span className="text-white/40 border-b border-white/10 ml-1">sign in</span>
                  </button>
                </div>

                <button type="submit" disabled={!email.includes('@')} className="animate-element animate-delay-600 w-full h-[56px] bg-white text-black rounded-2xl font-bold text-sm hover:bg-[#F5F5F5] hover:-translate-y-0.5 transition-all flex items-center justify-center gap-2 shadow-[0_20px_40px_rgba(255,255,255,0.1)] disabled:opacity-50 active:scale-[0.98]">
          ...
          {step === 3 && (
            <motion.div key="step3" initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -10 }} transition={{ duration: 0.4 }} className="space-y-6">
                  ...
                  <div key={i} className="p-4 bg-white/[0.02] border border-white/[0.06] rounded-xl space-y-3 group hover:bg-white/[0.04] transition-all">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {item.link && (
                          <a href={item.link} target="_blank" rel="noopener noreferrer" className="mt-3 inline-flex items-center gap-1.5 text-[10px] text-white/40 hover:text-white transition-colors font-medium tracking-tight"><ExternalLink className="w-3 h-3" />{item.linkText}</a>
                        )}
                      </div>
                    </div>
                  </div>
              ...
              <button onClick={handleGoogleSignUp} disabled={isLoading} className="w-full h-14 bg-white text-black rounded-2xl font-bold text-sm hover:bg-[#F5F5F5] hover:-translate-y-0.5 transition-all flex items-center justify-center gap-3 shadow-[0_20px_40px_rgba(255,255,255,0.1)] disabled:opacity-50 active:scale-[0.98]">
