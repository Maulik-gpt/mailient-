...
export function ThinkingLayer({
    ...
    return (
        <div className="space-y-0">
            {visibleSteps.map((step) => {
                ...
                return (
                    <div key={step.id} className="group relative">
                        <div className="absolute left-[6px] top-8 bottom-0 w-px bg-gradient-to-b from-graphite-border-strong via-graphite-muted-2 to-transparent pointer-events-none" />
                        <button
                            onClick={() => toggleStep(step.id)}
                            className="flex items-center gap-3 py-2 w-full text-left rounded-lg transition-colors -mx-1 px-3 bg-graphite-surface hover:bg-graphite-surface-2 border border-graphite-border"
                        >
                            <span
                                className={[
                        ...
                        {expandedSteps.has(step.id) && (
                            <div className="ml-4 pl-3 border-l border-graphite-border py-2 mb-1">
                                <p className="text-graphite-muted text-xs leading-relaxed whitespace-pre-wrap">{detailText}</p>
                            </div>
                        )}
                    </div>
                );
            ...
            {currentThought && (
                <p className="text-graphite-muted text-xs leading-relaxed pt-2">{currentThought}</p>
            )}
...
export function ArtifactCard({ type, title, version = 'v1', onView }: ArtifactCardProps) {
    return (
        <button
            onClick={onView}
            className="group flex items-center gap-3 bg-graphite-surface hover:bg-graphite-surface-2 border border-graphite-border hover:border-graphite-border-strong rounded-xl px-4 py-3 mt-3 mb-1 w-full max-w-sm transition-all duration-200"
        >
            ...
            <div className="flex items-center gap-2 flex-1 min-w-0">
                ...
                <span className="text-graphite-muted text-xs">{version}</span>
            </div>
