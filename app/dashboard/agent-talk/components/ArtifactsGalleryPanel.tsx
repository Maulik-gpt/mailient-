...
export function ArtifactsGalleryPanel({
  ...
  return (
    <div
      className={cn(
        'h-[calc(100vh-32px)] flex flex-col flex-shrink-0 relative bg-arcus-bg dark:bg-arcus-bg-elevated border-l border-arcus-border shadow-2xl rounded-r-[32px] overflow-hidden'
      )}
    ...
    >
      ...
      <div className="px-6 py-5 border-b border-arcus-border flex items-center justify-between shrink-0 relative z-10">
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-xl bg-arcus-surface border border-arcus-border flex items-center justify-center">
            <Library className="w-4 h-4 text-arcus-fg-secondary" />
          </div>
          <div>
            <h3 className="text-[14px] font-bold text-arcus-fg tracking-tight lowercase">Library</h3>
            {isLoadingAll ? (
              ...
              <p className="text-[10px] text-arcus-fg-secondary tracking-tight uppercase">All conversations</p>
            )}
          </div>
        </div>
        ...
        <button
          onClick={onClose}
          className="p-2 hover:bg-arcus-raised dark:hover:bg-arcus-surface-hover rounded-xl transition-all text-arcus-fg-secondary dark:text-arcus-fg-tertiary hover:text-arcus-fg dark:hover:text-white"
        >
          <X className="w-4 h-4" />
        </button>
      </div>
        ...
        <div className="px-6 py-4 border-b border-arcus-border shrink-0 relative z-10">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2 bg-arcus-surface border border-arcus-border rounded-xl text-[13px] text-arcus-fg placeholder:text-arcus-fg-muted focus:outline-none focus:border-arcus-fg-tertiary dark:focus:border-white/20 transition-all font-medium tracking-tight"
            />
          </div>
        </div>
      )}
      ...
      <div className="flex-1 overflow-y-auto px-6 py-6 relative z-10 select-none scrollbar-thin scrollbar-thumb-arcus-surface-hover">
        ...
        {dynamicArtifacts.length > 0 ? (
              ...
              {filteredArtifacts.map((art) => {
                ...
                return (
                  <div
                    ...
                    className="group p-4 bg-arcus-surface hover:bg-arcus-surface-hover border border-arcus-border hover:border-arcus-divider rounded-2xl transition-all duration-200 ease-out cursor-pointer relative"
                  >
                    {/* Top row: tag + download */}
                    <div className="flex items-center justify-between mb-2.5">
                      <span className="px-2 py-0.5 rounded-full bg-arcus-surface border border-arcus-border text-[9px] font-mono text-arcus-fg-secondary tracking-wide">
                        {art.tag}
                      </span>
                      <button
                        onClick={(e) => handleDownload(e, art)}
                        className="p-1.5 hover:bg-arcus-raised dark:hover:bg-arcus-surface-hover rounded-lg transition-colors text-arcus-fg-muted hover:text-arcus-fg-secondary opacity-0 group-hover:opacity-100"
                        title="Download .docx"
                      >
                        {isDownloading
                    ...
                    {/* Title */}
                    <h5 className="text-[13px] font-semibold text-arcus-fg leading-snug tracking-tight mb-1.5">
                      {art.title}
                    </h5>

                    {/* Content preview */}
                    {snippet && (
                      <p className="text-[12px] text-arcus-fg-secondary dark:text-arcus-fg-muted leading-relaxed line-clamp-2 mb-3">
                        {snippet}{snippet.length === 120 ? '…' : ''}
                      </p>
                    )}

                    {/* Footer */}
                    <div className="flex items-center justify-between pt-2.5 border-t border-arcus-border text-[10px] text-arcus-fg-secondary dark:text-arcus-fg-muted font-mono">
                      <span className="truncate max-w-[55%]">{art.subtitle}</span>
                      <span className="flex items-center gap-1 shrink-0 text-arcus-fg-muted dark:text-arcus-fg-tertiary group-hover:text-arcus-fg-secondary transition-colors">
          ...
          <div className="h-full flex flex-col items-center justify-center text-center px-6 py-12">
            <div className="w-16 h-16 rounded-2xl bg-arcus-surface border border-arcus-border flex items-center justify-center mb-6">
              <Sparkles className="w-6 h-6 text-arcus-fg-muted animate-pulse" />
            </div>
            
            <h4 className="text-arcus-fg text-[15px] font-bold tracking-tight mb-2 lowercase">
