...
function NotionPagePreview({ title }: { title: string }) {
  return (
    <div className="w-[140px] flex-shrink-0 bg-white/5 dark:bg-white/[0.04] border border-white/8 rounded-xl p-3 flex flex-col gap-2 select-none">
      <div className="flex items-center gap-1.5">
        <span className="text-[15px]">📝</span>
        <span className="text-[11px] font-semibold text-white/80 truncate leading-tight">{title}</span>
      </div>
      <div className="space-y-1.5 mt-0.5">
        <div className="h-[5px] bg-white/15 rounded-full w-full" />
        <div className="h-[5px] bg-white/15 rounded-full w-[85%]" />
        <div className="h-[5px] bg-white/10 rounded-full w-[60%]" />
        <div className="h-[5px] bg-white/8 rounded-full w-[75%]" />
      </div>
    </div>
  );
}
...
function CalendarPreview({ title, startTime }: { title: string; startTime?: string }) {
  ...
  return (
    <div className="w-[140px] flex-shrink-0 bg-white/5 dark:bg-white/[0.04] border border-white/8 rounded-xl p-3 flex flex-col gap-2 select-none">
      ...
      {formatted && (
        <p className="text-[10px] text-white/40 font-mono leading-tight">{formatted}</p>
      )}
      <div className="space-y-1.5">
        <div className="h-[5px] bg-white/15 rounded-full w-full" />
        <div className="h-[5px] bg-white/10 rounded-full w-[70%]" />
      </div>
    </div>
  );
}
...
export function ActionResultCard({ data, onUndo }: ActionResultCardProps) {
  ...
  return (
    <motion.div
      ...
      className="mt-3 mb-1 flex items-stretch gap-3 p-4 bg-white/[0.04] dark:bg-white/[0.03] border border-white/8 rounded-2xl overflow-hidden"
    >
      {/* Left: info + actions */}
      <div className="flex-1 flex flex-col justify-between min-w-0 gap-3">
        <div>
          ...
          <h4 className="text-[15px] font-bold text-white/90 leading-snug truncate">
            {data.title}
          </h4>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {primaryUrl && (
            <a
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 bg-white text-black text-[12px] font-semibold rounded-lg hover:bg-white/90 active:scale-95 transition-all"
            >
          ...
          {isCalendar && data.meetLink && (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-1.5 px-3 py-1.5 bg-white/8 text-white/80 text-[12px] font-semibold rounded-lg hover:bg-white/12 active:scale-95 transition-all border border-white/10"
            >
          ...
          {onUndo && (
            <button
              onClick={onUndo}
              title="Undo"
              className="p-1.5 rounded-lg text-white/30 hover:text-white/60 hover:bg-white/6 transition-all"
            >
