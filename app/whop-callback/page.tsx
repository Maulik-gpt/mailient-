...
function PaymentCallbackContent() {
    ...
    useEffect(() => {
        // Whop may pass these parameters after successful checkout
        const membershipId = searchParams.get('membership_id');
        const checkoutId = searchParams.get('checkout_id');
        const productId = searchParams.get('product_id');
        const plan = searchParams.get('plan');

        // Determine plan from product ID if available
        let detectedPlan = plan || localStorage.getItem('pending_plan') || 'starter';

        // Check product ID to determine plan
        if (productId === 'plan_OXtDPFaYlmYWN') {
            detectedPlan = 'starter';
        } else if (productId === 'plan_HjjXVb5SWxdOK') {
            detectedPlan = 'pro';
        }

        setPlanName(detectedPlan === 'pro' ? 'Pro' : 'Starter');
        ...
        const activateSubscription = async () => {
            try {
                ...
                // Redirect to home-feed after 3 seconds
                setTimeout(() => {
