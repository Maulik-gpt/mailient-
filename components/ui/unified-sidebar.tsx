...
export function UnifiedSidebar({ 
  ...
  const universalNavItems: SidebarItem[] = [
    ...
    { id: 'community', name: 'Community', icon: Users, isRoute: true, route: '/community' },
    { id: 'profile', name: 'Profile', icon: User, isRoute: true, route: '/dashboard/profile-bubble' },
  ];
        ...
        <div className={styles.container}>
          <div className={styles.nav}>
            {universalNavItems.map((item) => {
              ...
              // Special handling for profile icon with Gmail photo
              if (item.id === 'profile') {
                return (
                  <Tooltip delayDuration={100} key={item.id}>
                    <TooltipTrigger asChild>
                      ...
                      >
                        ...
                        <Icon className={styles.navIcon} style={{ display: profilePicture ? 'none' : 'block' }} />
                      </button>
                    </TooltipTrigger>
                    <TooltipContent side="right">
                      <p>{item.name}</p>
      ...
      {variant !== 'settings' && (
        <div className={styles.header}>
          <h1 className={styles.title}>
            {variant === 'dashboard' ? 'Gmail' : 'Sift AI Mail'}
          </h1>
          {variant === 'home-feed' && (
            <p className={styles.subtitle}>Smart Gmail Interface</p>
          )}
        </div>
      )}
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center w-full p-2 text-sm rounded-lg transition-colors ${
                        isActive ? styles.navItemActive : 'text-gray-300 hover:text-white hover:bg-gray-800'
                      }`}
                    >
